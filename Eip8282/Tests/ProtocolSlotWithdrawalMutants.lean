...
/-- Capella:452/480. The seventh payload's first sweep is stamped at
`start+25`, not restarted at 0. -/
theorem seventh_payload_sweep_index_is_not_restart :
    (seventhPayloadContinueSweep 0).index ≠
      (seventhPayloadContinueSweepRestart 0).index :=
  seventhPayloadContinueSweep_ne_restart 0

/-- Mutant: stamp from Gloas:2016 visits=2. -/
theorem seventh_payload_sweep_index_is_not_visits :
    (seventhPayloadContinueSweep 0).index ≠
      (seventhPayloadContinueSweepFromVisits 0).index :=
  seventhPayloadContinueSweep_ne_visits 0

/-- Mutant: stamp from the 23-chain cursor. -/
theorem seventh_payload_sweep_index_is_not_twentythree :
    (seventhPayloadContinueSweep 0).index ≠
      (seventhPayloadContinueSweepFromTwentyThree 0).index :=
  seventhPayloadContinueSweep_ne_twentythree 0

/-- Mutant: freeze after the sixth payload's first continued sweep. -/
theorem seventh_payload_sweep_index_is_not_twentyfour :
    (seventhPayloadContinueSweep 0).index ≠
      (seventhPayloadContinueSweepFromTwentyFour 0).index :=
  seventhPayloadContinueSweep_ne_twentyfour 0

/-- This payload's sweep cursor is `FLAG`, not the first payload `1|FLAG`. -/
theorem seventh_payload_sweep_validator_is_not_first_payload :
    (seventhPayloadContinueSweep 0).validatorIndex ≠
      (firstPayloadSkipTakeBreakCapWithdrawal 0).validatorIndex :=
  seventhPayloadContinueSweep_ne_first_payload_validator 0

/-- Mutant: reuse the sixth payload's second-sweep `1|FLAG`. -/
theorem seventh_payload_sweep_validator_is_not_sixth_second :
    (seventhPayloadContinueSweep 0).validatorIndex ≠
      (sixthPayloadContinueSweepSecond 0).validatorIndex :=
  seventhPayloadContinueSweep_ne_sixth_second_validator 0

/-- Capella:458 / Gloas:1868. The seventh payload's second sweep is
`start+26`, not frozen at `start+25`. -/
theorem seventh_payload_sweep_second_index_is_start_plus_26 :
    (seventhPayloadContinueSweepSecond 0).index = 26 :=
  seventhPayloadContinueSweepSecond_index 0

theorem seventh_payload_sweep_second_is_not_frozen :
    (seventhPayloadContinueSweepSecond 0).index ≠
      (seventhPayloadContinueSweepSecondFrozen 0).index :=
  seventhPayloadContinueSweepSecond_ne_frozen 0

theorem seventh_payload_sweep_second_is_not_this_first :
    (seventhPayloadContinueSweepSecond 0).index ≠
      (seventhPayloadContinueSweep 0).index :=
  seventhPayloadContinueSweepSecond_ne_first 0

/-- Validator is `1|FLAG`, not this payload's first `FLAG`. -/
theorem seventh_payload_sweep_second_validator_is_not_this_first :
    (seventhPayloadContinueSweepSecond 0).validatorIndex ≠
      (seventhPayloadContinueSweep 0).validatorIndex :=
  seventhPayloadContinueSweepSecond_ne_first_validator 0

/-- Validator is `1|FLAG`, not raw builder 1. -/
theorem seventh_payload_sweep_second_validator_is_not_raw :
    (seventhPayloadContinueSweepSecond 0).validatorIndex ≠ 1 :=
  seventhPayloadContinueSweepSecond_ne_raw 0

/-- Frozen continued pair of the seventh payload repeats `start+25`. -/
theorem seventh_payload_continued_pair_is_not_frozen :
    (firstPayloadTwoExitedWithdrawals 25).map (fun w => w.index) ≠
      (firstPayloadTwoExitedWithdrawalsFrozenIndex 25).map (fun w => w.index) :=
  seventhPayloadContinueSweep_pair_ne_frozen 0

/-- Capella:510 then 458. The seven-payload chain is `indexSeq start 27`,
not the 25-item omit. -/
theorem seven_payload_twentyseven_is_not_omit :
    indexSeq 0 27 ≠ indexSeq 0 25 := by
  intro h
  have := congrArg List.length h
  simp [indexSeq_length] at this

/-- Capella:506-510. After 27 appends the cursor is 27, not 25 or 26. -/
theorem seven_payload_cursor_is_not_omit :
    updateNextWithdrawalIndex 0 (indexSeq 0 27) ≠ 25 := by
  rw [updateNextWithdrawalIndex_seq (by decide : 0 < 27)]
  decide

theorem seven_payload_cursor_is_not_frozen :
    updateNextWithdrawalIndex 0 (indexSeq 0 27) ≠ 26 := by
  rw [updateNextWithdrawalIndex_seq (by decide : 0 < 27)]
  decide

/-- Restarting the seventh pair at 0 repeats indices 0 and 1. -/
theorem seven_payload_restart_repeats_prefix :
    indexSeq 0 25 ++ indexSeq 0 2 ≠ indexSeq 0 27 := by
  intro h
  have hsplit :
      indexSeq 0 25 ++ indexSeq 0 2 =
        indexSeq 0 25 ++ indexSeq 25 2 := by
    rw [← indexSeq_append 0 25 2] at h
    exact h
  have hcancel := List.append_cancel_left hsplit
  exact (by decide : 0 ≠ 25) (indexSeq_start_inj (by decide : 0 < 2) hcancel)

/-- fork.py:1118. The 27-chain is queues plus 79 Gwei, not the
25-chain's queues plus 67 Gwei. -/
theorem twentyseven_chain_credits_are_not_twentyfive :
    79 * GWEI_TO_WEI ≠ 67 * GWEI_TO_WEI := by
  simp [GWEI_TO_WEI]

/-- fork.py:1118. The 27-chain is not the 25-chain credit
(`items b1 + 60e9`). -/
theorem twentyseven_chain_credits_are_not_five_pairs :
    72 * GWEI_TO_WEI ≠ 60 * GWEI_TO_WEI := by
  simp [GWEI_TO_WEI]

/-- Capella:452/480. The eighth payload's first sweep is stamped at
`start+27`, not restarted at 0. -/
theorem eighth_payload_sweep_index_is_not_restart :
    (eighthPayloadContinueSweep 0).index ≠
      (eighthPayloadContinueSweepRestart 0).index :=
  eighthPayloadContinueSweep_ne_restart 0

/-- Mutant: stamp from Gloas:2016 visits=2. -/
theorem eighth_payload_sweep_index_is_not_visits :
    (eighthPayloadContinueSweep 0).index ≠
      (eighthPayloadContinueSweepFromVisits 0).index :=
  eighthPayloadContinueSweep_ne_visits 0

/-- Mutant: stamp from the 25-chain cursor. -/
theorem eighth_payload_sweep_index_is_not_twentyfive :
    (eighthPayloadContinueSweep 0).index ≠
      (eighthPayloadContinueSweepFromTwentyFive 0).index :=
  eighthPayloadContinueSweep_ne_twentyfive 0

/-- Mutant: freeze after the seventh payload's first continued sweep. -/
theorem eighth_payload_sweep_index_is_not_twentysix :
    (eighthPayloadContinueSweep 0).index ≠
      (eighthPayloadContinueSweepFromTwentySix 0).index :=
  eighthPayloadContinueSweep_ne_twentysix 0

/-- This payload's sweep cursor is `FLAG`, not the first payload `1|FLAG`. -/
theorem eighth_payload_sweep_validator_is_not_first_payload :
    (eighthPayloadContinueSweep 0).validatorIndex ≠
      (firstPayloadSkipTakeBreakCapWithdrawal 0).validatorIndex :=
  eighthPayloadContinueSweep_ne_first_payload_validator 0

/-- Mutant: reuse the seventh payload's second-sweep `1|FLAG`. -/
theorem eighth_payload_sweep_validator_is_not_seventh_second :
    (eighthPayloadContinueSweep 0).validatorIndex ≠
      (seventhPayloadContinueSweepSecond 0).validatorIndex :=
...
/-- Capella:452/480. The tenth payload's first sweep is stamped at
`start+31`, not restarted at 0. -/
theorem tenth_payload_sweep_index_is_not_restart :
    (tenthPayloadContinueSweep 0).index ≠
      (tenthPayloadContinueSweepRestart 0).index :=
  tenthPayloadContinueSweep_ne_restart 0

/-- Mutant: stamp from Gloas:2016 visits=2. -/
theorem tenth_payload_sweep_index_is_not_visits :
    (tenthPayloadContinueSweep 0).index ≠
      (tenthPayloadContinueSweepFromVisits 0).index :=
  tenthPayloadContinueSweep_ne_visits 0

/-- Mutant: stamp from the 29-chain cursor. -/
theorem tenth_payload_sweep_index_is_not_twentynine :
    (tenthPayloadContinueSweep 0).index ≠
      (tenthPayloadContinueSweepFromTwentyNine 0).index :=
  tenthPayloadContinueSweep_ne_twentynine 0

/-- Mutant: freeze after the ninth payload's first continued sweep. -/
theorem tenth_payload_sweep_index_is_not_thirty :
    (tenthPayloadContinueSweep 0).index ≠
      (tenthPayloadContinueSweepFromThirty 0).index :=
  tenthPayloadContinueSweep_ne_thirty 0

/-- This payload's sweep cursor is `FLAG`, not the first payload `1|FLAG`. -/
theorem tenth_payload_sweep_validator_is_not_first_payload :
    (tenthPayloadContinueSweep 0).validatorIndex ≠
      (firstPayloadSkipTakeBreakCapWithdrawal 0).validatorIndex :=
  tenthPayloadContinueSweep_ne_first_payload_validator 0

/-- Mutant: reuse the ninth payload's second-sweep `1|FLAG`. -/
theorem tenth_payload_sweep_validator_is_not_ninth_second :
    (tenthPayloadContinueSweep 0).validatorIndex ≠
      (ninthPayloadContinueSweepSecond 0).validatorIndex :=
...
/-- Capella:452/480. The eleventh payload's first sweep is stamped at
`start+33`, not restarted at 0. -/
theorem eleventh_payload_sweep_index_is_not_restart :
    (eleventhPayloadContinueSweep 0).index ≠
      (eleventhPayloadContinueSweepRestart 0).index :=
  eleventhPayloadContinueSweep_ne_restart 0

/-- Mutant: stamp from Gloas:2016 visits=2. -/
theorem eleventh_payload_sweep_index_is_not_visits :
    (eleventhPayloadContinueSweep 0).index ≠
      (eleventhPayloadContinueSweepFromVisits 0).index :=
  eleventhPayloadContinueSweep_ne_visits 0

/-- Mutant: stamp from the 31-chain cursor. -/
theorem eleventh_payload_sweep_index_is_not_thirtyone :
    (eleventhPayloadContinueSweep 0).index ≠
      (eleventhPayloadContinueSweepFromThirtyOne 0).index :=
  eleventhPayloadContinueSweep_ne_thirtyone 0

/-- Mutant: freeze after the tenth payload's first continued sweep. -/
theorem eleventh_payload_sweep_index_is_not_thirtytwo :
    (eleventhPayloadContinueSweep 0).index ≠
      (eleventhPayloadContinueSweepFromThirtyTwo 0).index :=
  eleventhPayloadContinueSweep_ne_thirtytwo 0

/-- This payload's sweep cursor is `FLAG`, not the first payload `1|FLAG`. -/
theorem eleventh_payload_sweep_validator_is_not_first_payload :
    (eleventhPayloadContinueSweep 0).validatorIndex ≠
      (firstPayloadSkipTakeBreakCapWithdrawal 0).validatorIndex :=
  eleventhPayloadContinueSweep_ne_first_payload_validator 0

/-- Mutant: reuse the tenth payload's second-sweep `1|FLAG`. -/
theorem eleventh_payload_sweep_validator_is_not_tenth_second :
    (eleventhPayloadContinueSweep 0).validatorIndex ≠
      (tenthPayloadContinueSweepSecond 0).validatorIndex :=
  eleventhPayloadContinueSweep_ne_tenth_second_validator 0

/-- Capella:458 / Gloas:1868. The eleventh payload's second sweep is
`start+34`, not frozen at `start+33`. -/
theorem eleventh_payload_sweep_second_index_is_start_plus_34 :
    (eleventhPayloadContinueSweepSecond 0).index = 34 :=
  eleventhPayloadContinueSweepSecond_index 0

theorem eleventh_payload_sweep_second_is_not_frozen :
    (eleventhPayloadContinueSweepSecond 0).index ≠
      (eleventhPayloadContinueSweepSecondFrozen 0).index :=
  eleventhPayloadContinueSweepSecond_ne_frozen 0

theorem eleventh_payload_sweep_second_is_not_this_first :
    (eleventhPayloadContinueSweepSecond 0).index ≠
      (eleventhPayloadContinueSweep 0).index :=
  eleventhPayloadContinueSweepSecond_ne_first 0

/-- Validator is `1|FLAG`, not this payload's first `FLAG`. -/
theorem eleventh_payload_sweep_second_validator_is_not_this_first :
    (eleventhPayloadContinueSweepSecond 0).validatorIndex ≠
      (eleventhPayloadContinueSweep 0).validatorIndex :=
  eleventhPayloadContinueSweepSecond_ne_first_validator 0

/-- Validator is `1|FLAG`, not raw builder 1. -/
theorem eleventh_payload_sweep_second_validator_is_not_raw :
    (eleventhPayloadContinueSweepSecond 0).validatorIndex ≠ 1 :=
  eleventhPayloadContinueSweepSecond_ne_raw 0

/-- Frozen continued pair of the eleventh payload repeats `start+33`. -/
theorem eleventh_payload_continued_pair_is_not_frozen :
    (firstPayloadTwoExitedWithdrawals 33).map (fun w => w.index) ≠
      (firstPayloadTwoExitedWithdrawalsFrozenIndex 33).map (fun w => w.index) :=
  eleventhPayloadContinueSweep_pair_ne_frozen 0

/-- Capella:510 then 458. The eleven-payload chain is `indexSeq start 35`,
not the 33-item omit. -/
theorem eleven_payload_thirtyfive_is_not_omit :
    indexSeq 0 35 ≠ indexSeq 0 33 := by
  intro h
  have := congrArg List.length h
  simp [indexSeq_length] at this

/-- Capella:506-510. After 35 appends the cursor is 35, not 33 or 34. -/
theorem eleven_payload_cursor_is_not_omit :
    updateNextWithdrawalIndex 0 (indexSeq 0 35) ≠ 33 := by
  rw [updateNextWithdrawalIndex_seq (by decide : 0 < 35)]
  decide

theorem eleven_payload_cursor_is_not_frozen :
    updateNextWithdrawalIndex 0 (indexSeq 0 35) ≠ 34 := by
  rw [updateNextWithdrawalIndex_seq (by decide : 0 < 35)]
  decide

/-- Restarting the eleventh pair at 0 repeats indices 0 and 1. -/
theorem eleven_payload_restart_repeats_prefix :
    indexSeq 0 33 ++ indexSeq 0 2 ≠ indexSeq 0 35 := by
  intro h
  have hsplit :
      indexSeq 0 33 ++ indexSeq 0 2 =
        indexSeq 0 33 ++ indexSeq 33 2 := by
    rw [← indexSeq_append 0 33 2] at h
    exact h
  have hcancel := List.append_cancel_left hsplit
  exact (by decide : 0 ≠ 33) (indexSeq_start_inj (by decide : 0 < 2) hcancel)

/-- fork.py:1118. The 35-chain is queues plus 127 Gwei, not the
33-chain's queues plus 115 Gwei. -/
theorem thirtyfive_chain_credits_are_not_thirtythree :
    127 * GWEI_TO_WEI ≠ 115 * GWEI_TO_WEI := by
  simp [GWEI_TO_WEI]

/-- fork.py:1118. The 35-chain is not the 33-chain credit
(`items b1 + 108e9`). -/
theorem thirtyfive_chain_credits_are_not_nine_pairs :
    120 * GWEI_TO_WEI ≠ 108 * GWEI_TO_WEI := by
  simp [GWEI_TO_WEI]



/-- Capella:452/480. The twelfth payload's first sweep is stamped at
`start+35`, not restarted at 0. -/
theorem twelfth_payload_sweep_index_is_not_restart :
    (twelfthPayloadContinueSweep 0).index ≠
      (twelfthPayloadContinueSweepRestart 0).index :=
  twelfthPayloadContinueSweep_ne_restart 0

/-- Mutant: stamp from Gloas:2016 visits=2. -/
theorem twelfth_payload_sweep_index_is_not_visits :
    (twelfthPayloadContinueSweep 0).index ≠
      (twelfthPayloadContinueSweepFromVisits 0).index :=
  twelfthPayloadContinueSweep_ne_visits 0

/-- Mutant: stamp from the 33-chain cursor. -/
theorem twelfth_payload_sweep_index_is_not_thirtythree :
    (twelfthPayloadContinueSweep 0).index ≠
      (twelfthPayloadContinueSweepFromThirtyThree 0).index :=
  twelfthPayloadContinueSweep_ne_thirtythree 0

/-- Mutant: freeze after the eleventh payload's first continued sweep. -/
theorem twelfth_payload_sweep_index_is_not_thirtyfour :
    (twelfthPayloadContinueSweep 0).index ≠
      (twelfthPayloadContinueSweepFromThirtyFour 0).index :=
  twelfthPayloadContinueSweep_ne_thirtyfour 0

/-- This payload's sweep cursor is `FLAG`, not the first payload `1|FLAG`. -/
theorem twelfth_payload_sweep_validator_is_not_first_payload :
    (twelfthPayloadContinueSweep 0).validatorIndex ≠
      (firstPayloadSkipTakeBreakCapWithdrawal 0).validatorIndex :=
  twelfthPayloadContinueSweep_ne_first_payload_validator 0

/-- Mutant: reuse the eleventh payload's second-sweep `1|FLAG`. -/
theorem twelfth_payload_sweep_validator_is_not_eleventh_second :
    (twelfthPayloadContinueSweep 0).validatorIndex ≠
      (eleventhPayloadContinueSweepSecond 0).validatorIndex :=
...
/-- Capella:458 / Gloas:1868. The thirteenth payload's second sweep is
`start+38`, not frozen at `start+37`. -/
theorem thirteenth_payload_sweep_second_index_is_start_plus_38 :
    (thirteenthPayloadContinueSweepSecond 0).index = 38 :=
  thirteenthPayloadContinueSweepSecond_index 0

theorem thirteenth_payload_sweep_second_is_not_frozen :
    (thirteenthPayloadContinueSweepSecond 0).index ≠
      (thirteenthPayloadContinueSweepSecondFrozen 0).index :=
  thirteenthPayloadContinueSweepSecond_ne_frozen 0

theorem thirteenth_payload_sweep_second_is_not_this_first :
    (thirteenthPayloadContinueSweepSecond 0).index ≠
      (thirteenthPayloadContinueSweep 0).index :=
  thirteenthPayloadContinueSweepSecond_ne_first 0

/-- Validator is `1|FLAG`, not this payload's first `FLAG`. -/
theorem thirteenth_payload_sweep_second_validator_is_not_this_first :
    (thirteenthPayloadContinueSweepSecond 0).validatorIndex ≠
      (thirteenthPayloadContinueSweep 0).validatorIndex :=
  thirteenthPayloadContinueSweepSecond_ne_first_validator 0

/-- Validator is `1|FLAG`, not raw builder 1. -/
theorem thirteenth_payload_sweep_second_validator_is_not_raw :
    (thirteenthPayloadContinueSweepSecond 0).validatorIndex ≠ 1 :=
  thirteenthPayloadContinueSweepSecond_ne_raw 0

/-- Frozen continued pair of the thirteenth payload repeats `start+37`. -/
theorem thirteenth_payload_continued_pair_is_not_frozen :
    (firstPayloadTwoExitedWithdrawals 37).map (fun w => w.index) ≠
      (firstPayloadTwoExitedWithdrawalsFrozenIndex 37).map (fun w => w.index) :=
  thirteenthPayloadContinueSweep_pair_ne_frozen 0

/-- Capella:510 then 458. The thirteen-payload chain is `indexSeq start 39`,
not the 37-item omit. -/
theorem thirteen_payload_thirtynine_is_not_omit :
    indexSeq 0 39 ≠ indexSeq 0 37 := by
  intro h
  have := congrArg List.length h
  simp [indexSeq_length] at this

/-- Capella:506-510. After 39 appends the cursor is 39, not 37 or 38. -/
theorem thirteen_payload_cursor_is_not_omit :
    updateNextWithdrawalIndex 0 (indexSeq 0 39) ≠ 37 := by
  rw [updateNextWithdrawalIndex_seq (by decide : 0 < 39)]
  decide

theorem thirteen_payload_cursor_is_not_frozen :
    updateNextWithdrawalIndex 0 (indexSeq 0 39) ≠ 38 := by
  rw [updateNextWithdrawalIndex_seq (by decide : 0 < 39)]
  decide

/-- Restarting the thirteenth pair at 0 repeats indices 0 and 1. -/
theorem thirteen_payload_restart_repeats_prefix :
    indexSeq 0 37 ++ indexSeq 0 2 ≠ indexSeq 0 39 := by
  intro h
  have hsplit :
      indexSeq 0 37 ++ indexSeq 0 2 =
        indexSeq 0 37 ++ indexSeq 37 2 := by
    rw [← indexSeq_append 0 37 2] at h
    exact h
  have hcancel := List.append_cancel_left hsplit
  exact (by decide : 0 ≠ 37) (indexSeq_start_inj (by decide : 0 < 2) hcancel)

/-- fork.py:1118. The 39-chain is queues plus 151 Gwei, not the
37-chain's queues plus 139 Gwei. -/
theorem thirtynine_chain_credits_are_not_thirtyseven :
    151 * GWEI_TO_WEI ≠ 139 * GWEI_TO_WEI := by
  simp [GWEI_TO_WEI]

/-- fork.py:1118. The 39-chain is not the 37-chain credit
(`items b1 + 132e9`). -/
theorem thirtynine_chain_credits_are_not_eleven_pairs :
    144 * GWEI_TO_WEI ≠ 132 * GWEI_TO_WEI := by
  simp [GWEI_TO_WEI]



/-- Capella:452/480. The fourteenth payload's first sweep is stamped at
`start+39`, not restarted at 0. -/
theorem fourteenth_payload_sweep_index_is_not_restart :
    (fourteenthPayloadContinueSweep 0).index ≠
      (fourteenthPayloadContinueSweepRestart 0).index :=
  fourteenthPayloadContinueSweep_ne_restart 0

/-- Mutant: stamp from Gloas:2016 visits=2. -/
theorem fourteenth_payload_sweep_index_is_not_visits :
    (fourteenthPayloadContinueSweep 0).index ≠
      (fourteenthPayloadContinueSweepFromVisits 0).index :=
  fourteenthPayloadContinueSweep_ne_visits 0

/-- Mutant: stamp from the 37-chain cursor. -/
theorem fourteenth_payload_sweep_index_is_not_thirtyseven :
    (fourteenthPayloadContinueSweep 0).index ≠
      (fourteenthPayloadContinueSweepFromThirtySeven 0).index :=
  fourteenthPayloadContinueSweep_ne_thirtyseven 0

/-- Mutant: freeze after the thirteenth payload's first continued sweep. -/
theorem fourteenth_payload_sweep_index_is_not_thirtyeight :
    (fourteenthPayloadContinueSweep 0).index ≠
      (fourteenthPayloadContinueSweepFromThirtyEight 0).index :=
  fourteenthPayloadContinueSweep_ne_thirtyeight 0

/-- This payload's sweep cursor is `FLAG`, not the first payload `1|FLAG`. -/
theorem fourteenth_payload_sweep_validator_is_not_first_payload :
    (fourteenthPayloadContinueSweep 0).validatorIndex ≠
      (firstPayloadSkipTakeBreakCapWithdrawal 0).validatorIndex :=
  fourteenthPayloadContinueSweep_ne_first_payload_validator 0

/-- Mutant: reuse the thirteenth payload's second-sweep `1|FLAG`. -/
theorem fourteenth_payload_sweep_validator_is_not_thirteenth_second :
    (fourteenthPayloadContinueSweep 0).validatorIndex ≠
      (thirteenthPayloadContinueSweepSecond 0).validatorIndex :=
  fourteenthPayloadContinueSweep_ne_thirteenth_second_validator 0

/-- Capella:458 / Gloas:1868. The fourteenth payload's second sweep is
`start+40`, not frozen at `start+39`. -/
theorem fourteenth_payload_sweep_second_index_is_start_plus_40 :
    (fourteenthPayloadContinueSweepSecond 0).index = 40 :=
  fourteenthPayloadContinueSweepSecond_index 0

theorem fourteenth_payload_sweep_second_is_not_frozen :
    (fourteenthPayloadContinueSweepSecond 0).index ≠
      (fourteenthPayloadContinueSweepSecondFrozen 0).index :=
  fourteenthPayloadContinueSweepSecond_ne_frozen 0

theorem fourteenth_payload_sweep_second_is_not_this_first :
    (fourteenthPayloadContinueSweepSecond 0).index ≠
      (fourteenthPayloadContinueSweep 0).index :=
  fourteenthPayloadContinueSweepSecond_ne_first 0

/-- Validator is `1|FLAG`, not this payload's first `FLAG`. -/
theorem fourteenth_payload_sweep_second_validator_is_not_this_first :
    (fourteenthPayloadContinueSweepSecond 0).validatorIndex ≠
      (fourteenthPayloadContinueSweep 0).validatorIndex :=
  fourteenthPayloadContinueSweepSecond_ne_first_validator 0

/-- Validator is `1|FLAG`, not raw builder 1. -/
theorem fourteenth_payload_sweep_second_validator_is_not_raw :
    (fourteenthPayloadContinueSweepSecond 0).validatorIndex ≠ 1 :=
  fourteenthPayloadContinueSweepSecond_ne_raw 0

/-- Frozen continued pair of the fourteenth payload repeats `start+39`. -/
theorem fourteenth_payload_continued_pair_is_not_frozen :
    (firstPayloadTwoExitedWithdrawals 39).map (fun w => w.index) ≠
      (firstPayloadTwoExitedWithdrawalsFrozenIndex 39).map (fun w => w.index) :=
  fourteenthPayloadContinueSweep_pair_ne_frozen 0

/-- Capella:510 then 458. The fourteen-payload chain is `indexSeq start 41`,
not the 39-item omit. -/
theorem fourteen_payload_fortyone_is_not_omit :
    indexSeq 0 41 ≠ indexSeq 0 39 := by
  intro h
  have := congrArg List.length h
  simp [indexSeq_length] at this

/-- Capella:506-510. After 41 appends the cursor is 41, not 39 or 40. -/
theorem fourteen_payload_cursor_is_not_omit :
    updateNextWithdrawalIndex 0 (indexSeq 0 41) ≠ 39 := by
  rw [updateNextWithdrawalIndex_seq (by decide : 0 < 41)]
  decide

theorem fourteen_payload_cursor_is_not_frozen :
    updateNextWithdrawalIndex 0 (indexSeq 0 41) ≠ 40 := by
  rw [updateNextWithdrawalIndex_seq (by decide : 0 < 41)]
  decide

/-- Restarting the fourteenth pair at 0 repeats indices 0 and 1. -/
theorem fourteen_payload_restart_repeats_prefix :
    indexSeq 0 39 ++ indexSeq 0 2 ≠ indexSeq 0 41 := by
  intro h
  have hsplit :
      indexSeq 0 39 ++ indexSeq 0 2 =
        indexSeq 0 39 ++ indexSeq 39 2 := by
    rw [← indexSeq_append 0 39 2] at h
    exact h
  have hcancel := List.append_cancel_left hsplit
  exact (by decide : 0 ≠ 39) (indexSeq_start_inj (by decide : 0 < 2) hcancel)

/-- fork.py:1118. The 41-chain is queues plus 163 Gwei, not the
39-chain's queues plus 151 Gwei. -/
theorem fortyone_chain_credits_are_not_thirtynine :
    163 * GWEI_TO_WEI ≠ 151 * GWEI_TO_WEI := by
  simp [GWEI_TO_WEI]

/-- fork.py:1118. The 41-chain is not the 39-chain credit
(`items b1 + 144e9`). -/
theorem fortyone_chain_credits_are_not_twelve_pairs :
    156 * GWEI_TO_WEI ≠ 144 * GWEI_TO_WEI := by
  simp [GWEI_TO_WEI]



/-- Capella:452/480. The fifteenth payload's first sweep is stamped at
`start+41`, not restarted at 0. -/
theorem fifteenth_payload_sweep_index_is_not_restart :
    (fifteenthPayloadContinueSweep 0).index ≠
      (fifteenthPayloadContinueSweepRestart 0).index :=
  fifteenthPayloadContinueSweep_ne_restart 0

/-- Mutant: stamp from Gloas:2016 visits=2. -/
theorem fifteenth_payload_sweep_index_is_not_visits :
    (fifteenthPayloadContinueSweep 0).index ≠
      (fifteenthPayloadContinueSweepFromVisits 0).index :=
  fifteenthPayloadContinueSweep_ne_visits 0

/-- Mutant: stamp from the 39-chain cursor. -/
theorem fifteenth_payload_sweep_index_is_not_thirtynine :
    (fifteenthPayloadContinueSweep 0).index ≠
      (fifteenthPayloadContinueSweepFromThirtyNine 0).index :=
  fifteenthPayloadContinueSweep_ne_thirtynine 0

/-- Mutant: freeze after the fourteenth payload's second continued sweep. -/
theorem fifteenth_payload_sweep_index_is_not_forty :
    (fifteenthPayloadContinueSweep 0).index ≠
      (fifteenthPayloadContinueSweepFromForty 0).index :=
  fifteenthPayloadContinueSweep_ne_forty 0

/-- This payload's sweep cursor is `FLAG`, not the first payload `1|FLAG`. -/
theorem fifteenth_payload_sweep_validator_is_not_first_payload :
    (fifteenthPayloadContinueSweep 0).validatorIndex ≠
      (firstPayloadSkipTakeBreakCapWithdrawal 0).validatorIndex :=
  fifteenthPayloadContinueSweep_ne_first_payload_validator 0

/-- Mutant: reuse the fourteenth payload's second-sweep `1|FLAG`. -/
theorem fifteenth_payload_sweep_validator_is_not_fourteenth_second :
    (fifteenthPayloadContinueSweep 0).validatorIndex ≠
      (fourteenthPayloadContinueSweepSecond 0).validatorIndex :=
  fifteenthPayloadContinueSweep_ne_fourteenth_second_validator 0

/-- Capella:458 / Gloas:1868. The fifteenth payload's second sweep is
`start+42`, not frozen at `start+41`. -/
theorem fifteenth_payload_sweep_second_index_is_start_plus_42 :
    (fifteenthPayloadContinueSweepSecond 0).index = 42 :=
  fifteenthPayloadContinueSweepSecond_index 0

theorem fifteenth_payload_sweep_second_is_not_frozen :
    (fifteenthPayloadContinueSweepSecond 0).index ≠
      (fifteenthPayloadContinueSweepSecondFrozen 0).index :=
  fifteenthPayloadContinueSweepSecond_ne_frozen 0

theorem fifteenth_payload_sweep_second_is_not_this_first :
    (fifteenthPayloadContinueSweepSecond 0).index ≠
      (fifteenthPayloadContinueSweep 0).index :=
  fifteenthPayloadContinueSweepSecond_ne_first 0

/-- Validator is `1|FLAG`, not this payload's first `FLAG`. -/
theorem fifteenth_payload_sweep_second_validator_is_not_this_first :
    (fifteenthPayloadContinueSweepSecond 0).validatorIndex ≠
      (fifteenthPayloadContinueSweep 0).validatorIndex :=
  fifteenthPayloadContinueSweepSecond_ne_first_validator 0

/-- Validator is `1|FLAG`, not raw builder 1. -/
theorem fifteenth_payload_sweep_second_validator_is_not_raw :
    (fifteenthPayloadContinueSweepSecond 0).validatorIndex ≠ 1 :=
  fifteenthPayloadContinueSweepSecond_ne_raw 0

/-- Frozen continued pair of the fifteenth payload repeats `start+41`. -/
theorem fifteenth_payload_continued_pair_is_not_frozen :
    (firstPayloadTwoExitedWithdrawals 41).map (fun w => w.index) ≠
      (firstPayloadTwoExitedWithdrawalsFrozenIndex 41).map (fun w => w.index) :=
  fifteenthPayloadContinueSweep_pair_ne_frozen 0

/-- Capella:510 then 458. The fifteen-payload chain is `indexSeq start 43`,
not the 41-item omit. -/
theorem fifteen_payload_fortythree_is_not_omit :
    indexSeq 0 43 ≠ indexSeq 0 41 := by
  intro h
  have := congrArg List.length h
  simp [indexSeq_length] at this

/-- Capella:506-510. After 43 appends the cursor is 43, not 41 or 42. -/
theorem fifteen_payload_cursor_is_not_omit :
    updateNextWithdrawalIndex 0 (indexSeq 0 43) ≠ 41 := by
  rw [updateNextWithdrawalIndex_seq (by decide : 0 < 43)]
  decide

theorem fifteen_payload_cursor_is_not_frozen :
    updateNextWithdrawalIndex 0 (indexSeq 0 43) ≠ 42 := by
  rw [updateNextWithdrawalIndex_seq (by decide : 0 < 43)]
  decide

/-- Restarting the fifteenth pair at 0 repeats indices 0 and 1. -/
theorem fifteen_payload_restart_repeats_prefix :
    indexSeq 0 41 ++ indexSeq 0 2 ≠ indexSeq 0 43 := by
  intro h
  have hsplit :
      indexSeq 0 41 ++ indexSeq 0 2 =
        indexSeq 0 41 ++ indexSeq 41 2 := by
    rw [← indexSeq_append 0 41 2] at h
    exact h
  have hcancel := List.append_cancel_left hsplit
  exact (by decide : 0 ≠ 41) (indexSeq_start_inj (by decide : 0 < 2) hcancel)

/-- fork.py:1118. The 43-chain is queues plus 175 Gwei, not the
41-chain's queues plus 163 Gwei. -/
theorem fortythree_chain_credits_are_not_fortyone :
    175 * GWEI_TO_WEI ≠ 163 * GWEI_TO_WEI := by
  simp [GWEI_TO_WEI]

/-- fork.py:1118. The 43-chain is not the 41-chain credit
(`items b1 + 156e9`). -/
theorem fortythree_chain_credits_are_not_thirteen_pairs :
    168 * GWEI_TO_WEI ≠ 156 * GWEI_TO_WEI := by
  simp [GWEI_TO_WEI]



/-- Capella:452/480. The sixteenth payload's first sweep is stamped at
`start+43`, not restarted at 0. -/
theorem sixteenth_payload_sweep_index_is_not_restart :
    (sixteenthPayloadContinueSweep 0).index ≠
      (sixteenthPayloadContinueSweepRestart 0).index :=
  sixteenthPayloadContinueSweep_ne_restart 0

/-- Mutant: stamp from Gloas:2016 visits=2. -/
theorem sixteenth_payload_sweep_index_is_not_visits :
    (sixteenthPayloadContinueSweep 0).index ≠
      (sixteenthPayloadContinueSweepFromVisits 0).index :=
  sixteenthPayloadContinueSweep_ne_visits 0

/-- Mutant: stamp from the 41-chain cursor. -/
theorem sixteenth_payload_sweep_index_is_not_fortyone :
    (sixteenthPayloadContinueSweep 0).index ≠
      (sixteenthPayloadContinueSweepFromFortyOne 0).index :=
  sixteenthPayloadContinueSweep_ne_fortyone 0

/-- Mutant: freeze after the fifteenth payload's second continued sweep. -/
theorem sixteenth_payload_sweep_index_is_not_fortytwo :
    (sixteenthPayloadContinueSweep 0).index ≠
      (sixteenthPayloadContinueSweepFromFortyTwo 0).index :=
  sixteenthPayloadContinueSweep_ne_fortytwo 0

/-- This payload's sweep cursor is `FLAG`, not the first payload `1|FLAG`. -/
theorem sixteenth_payload_sweep_validator_is_not_first_payload :
    (sixteenthPayloadContinueSweep 0).validatorIndex ≠
      (firstPayloadSkipTakeBreakCapWithdrawal 0).validatorIndex :=
  sixteenthPayloadContinueSweep_ne_first_payload_validator 0

/-- Mutant: reuse the fifteenth payload's second-sweep `1|FLAG`. -/
theorem sixteenth_payload_sweep_validator_is_not_fifteenth_second :
    (sixteenthPayloadContinueSweep 0).validatorIndex ≠
      (fifteenthPayloadContinueSweepSecond 0).validatorIndex :=
  sixteenthPayloadContinueSweep_ne_fifteenth_second_validator 0

/-- Capella:458 / Gloas:1868. The sixteenth payload's second sweep is
`start+44`, not frozen at `start+43`. -/
theorem sixteenth_payload_sweep_second_index_is_start_plus_44 :
    (sixteenthPayloadContinueSweepSecond 0).index = 44 :=
  sixteenthPayloadContinueSweepSecond_index 0

theorem sixteenth_payload_sweep_second_is_not_frozen :
    (sixteenthPayloadContinueSweepSecond 0).index ≠
      (sixteenthPayloadContinueSweepSecondFrozen 0).index :=
  sixteenthPayloadContinueSweepSecond_ne_frozen 0

theorem sixteenth_payload_sweep_second_is_not_this_first :
    (sixteenthPayloadContinueSweepSecond 0).index ≠
      (sixteenthPayloadContinueSweep 0).index :=
  sixteenthPayloadContinueSweepSecond_ne_first 0

/-- Validator is `1|FLAG`, not this payload's first `FLAG`. -/
theorem sixteenth_payload_sweep_second_validator_is_not_this_first :
    (sixteenthPayloadContinueSweepSecond 0).validatorIndex ≠
      (sixteenthPayloadContinueSweep 0).validatorIndex :=
  sixteenthPayloadContinueSweepSecond_ne_first_validator 0

/-- Validator is `1|FLAG`, not raw builder 1. -/
theorem sixteenth_payload_sweep_second_validator_is_not_raw :
    (sixteenthPayloadContinueSweepSecond 0).validatorIndex ≠ 1 :=
  sixteenthPayloadContinueSweepSecond_ne_raw 0

/-- Frozen continued pair of the sixteenth payload repeats `start+43`. -/
theorem sixteenth_payload_continued_pair_is_not_frozen :
    (firstPayloadTwoExitedWithdrawals 43).map (fun w => w.index) ≠
      (firstPayloadTwoExitedWithdrawalsFrozenIndex 43).map (fun w => w.index) :=
  sixteenthPayloadContinueSweep_pair_ne_frozen 0

/-- Capella:510 then 458. The sixteen-payload chain is `indexSeq start 45`,
not the 43-item omit. -/
theorem sixteen_payload_fortyfive_is_not_omit :
    indexSeq 0 45 ≠ indexSeq 0 43 := by
  intro h
  have := congrArg List.length h
  simp [indexSeq_length] at this

/-- Capella:506-510. After 45 appends the cursor is 45, not 43 or 44. -/
theorem sixteen_payload_cursor_is_not_omit :
    updateNextWithdrawalIndex 0 (indexSeq 0 45) ≠ 43 := by
  rw [updateNextWithdrawalIndex_seq (by decide : 0 < 45)]
  decide

theorem sixteen_payload_cursor_is_not_frozen :
    updateNextWithdrawalIndex 0 (indexSeq 0 45) ≠ 44 := by
  rw [updateNextWithdrawalIndex_seq (by decide : 0 < 45)]
  decide

/-- Restarting the sixteenth pair at 0 repeats indices 0 and 1. -/
theorem sixteen_payload_restart_repeats_prefix :
    indexSeq 0 43 ++ indexSeq 0 2 ≠ indexSeq 0 45 := by
  intro h
  have hsplit :
      indexSeq 0 43 ++ indexSeq 0 2 =
        indexSeq 0 43 ++ indexSeq 43 2 := by
    rw [← indexSeq_append 0 43 2] at h
    exact h
  have hcancel := List.append_cancel_left hsplit
  exact (by decide : 0 ≠ 43) (indexSeq_start_inj (by decide : 0 < 2) hcancel)

/-- fork.py:1118. The 45-chain is queues plus 187 Gwei, not the
43-chain's queues plus 175 Gwei. -/
theorem fortyfive_chain_credits_are_not_fortythree :
    187 * GWEI_TO_WEI ≠ 175 * GWEI_TO_WEI := by
  simp [GWEI_TO_WEI]

/-- fork.py:1118. The 45-chain is not the 43-chain credit
(`items b1 + 168e9`). -/
theorem fortyfive_chain_credits_are_not_fourteen_pairs :
    180 * GWEI_TO_WEI ≠ 168 * GWEI_TO_WEI := by
  simp [GWEI_TO_WEI]
...
/-- Capella:458 / Gloas:1868. The seventeenth payload's second sweep is
`start+46`, not frozen at `start+45`. -/
theorem seventeenth_payload_sweep_second_index_is_start_plus_46 :
    (seventeenthPayloadContinueSweepSecond 0).index = 46 :=
  seventeenthPayloadContinueSweepSecond_index 0

theorem seventeenth_payload_sweep_second_is_not_frozen :
    (seventeenthPayloadContinueSweepSecond 0).index ≠
      (seventeenthPayloadContinueSweepSecondFrozen 0).index :=
  seventeenthPayloadContinueSweepSecond_ne_frozen 0

theorem seventeenth_payload_sweep_second_is_not_this_first :
    (seventeenthPayloadContinueSweepSecond 0).index ≠
      (seventeenthPayloadContinueSweep 0).index :=
  seventeenthPayloadContinueSweepSecond_ne_first 0

/-- Validator is `1|FLAG`, not this payload's first `FLAG`. -/
theorem seventeenth_payload_sweep_second_validator_is_not_this_first :
    (seventeenthPayloadContinueSweepSecond 0).validatorIndex ≠
      (seventeenthPayloadContinueSweep 0).validatorIndex :=
  seventeenthPayloadContinueSweepSecond_ne_first_validator 0

/-- Validator is `1|FLAG`, not raw builder 1. -/
theorem seventeenth_payload_sweep_second_validator_is_not_raw :
    (seventeenthPayloadContinueSweepSecond 0).validatorIndex ≠ 1 :=
  seventeenthPayloadContinueSweepSecond_ne_raw 0

/-- Frozen continued pair of the seventeenth payload repeats `start+45`. -/
theorem seventeenth_payload_continued_pair_is_not_frozen :
    (firstPayloadTwoExitedWithdrawals 45).map (fun w => w.index) ≠
      (firstPayloadTwoExitedWithdrawalsFrozenIndex 45).map (fun w => w.index) :=
  seventeenthPayloadContinueSweep_pair_ne_frozen 0

/-- Capella:510 then 458. The seventeen-payload chain is `indexSeq start 47`,
not the 45-item omit. -/
theorem seventeen_payload_fortyseven_is_not_omit :
    indexSeq 0 47 ≠ indexSeq 0 45 := by
  intro h
  have := congrArg List.length h
  simp [indexSeq_length] at this

/-- Capella:506-510. After 47 appends the cursor is 47, not 45 or 46. -/
theorem seventeen_payload_cursor_is_not_omit :
    updateNextWithdrawalIndex 0 (indexSeq 0 47) ≠ 45 := by
  rw [updateNextWithdrawalIndex_seq (by decide : 0 < 47)]
  decide

theorem seventeen_payload_cursor_is_not_frozen :
    updateNextWithdrawalIndex 0 (indexSeq 0 47) ≠ 46 := by
  rw [updateNextWithdrawalIndex_seq (by decide : 0 < 47)]
  decide

/-- Restarting the seventeenth pair at 0 repeats indices 0 and 1. -/
theorem seventeen_payload_restart_repeats_prefix :
    indexSeq 0 45 ++ indexSeq 0 2 ≠ indexSeq 0 47 := by
  intro h
  have hsplit :
      indexSeq 0 45 ++ indexSeq 0 2 =
        indexSeq 0 45 ++ indexSeq 45 2 := by
    rw [← indexSeq_append 0 45 2] at h
    exact h
  have hcancel := List.append_cancel_left hsplit
  exact (by decide : 0 ≠ 45) (indexSeq_start_inj (by decide : 0 < 2) hcancel)

/-- fork.py:1118. The 47-chain is queues plus 199 Gwei, not the
45-chain's queues plus 187 Gwei. -/
theorem fortyseven_chain_credits_are_not_fortyfive :
    199 * GWEI_TO_WEI ≠ 187 * GWEI_TO_WEI := by
  simp [GWEI_TO_WEI]

/-- fork.py:1118. The 47-chain is not the 45-chain credit
(`items b1 + 180e9`). -/
theorem fortyseven_chain_credits_are_not_fifteen_pairs :
    192 * GWEI_TO_WEI ≠ 180 * GWEI_TO_WEI := by
  simp [GWEI_TO_WEI]



/-- Capella:452/480. The eighteenth payload's first sweep is stamped at
`start+47`, not restarted at 0. -/
theorem eighteenth_payload_sweep_index_is_not_restart :
    (eighteenthPayloadContinueSweep 0).index ≠
      (eighteenthPayloadContinueSweepRestart 0).index :=
  eighteenthPayloadContinueSweep_ne_restart 0

/-- Mutant: stamp from Gloas:2016 visits=2. -/
theorem eighteenth_payload_sweep_index_is_not_visits :
    (eighteenthPayloadContinueSweep 0).index ≠
      (eighteenthPayloadContinueSweepFromVisits 0).index :=
  eighteenthPayloadContinueSweep_ne_visits 0

/-- Mutant: stamp from the 45-chain cursor. -/
theorem eighteenth_payload_sweep_index_is_not_fortyfive :
    (eighteenthPayloadContinueSweep 0).index ≠
      (eighteenthPayloadContinueSweepFromFortyFive 0).index :=
  eighteenthPayloadContinueSweep_ne_fortyfive 0

/-- Mutant: freeze after the seventeenth payload's second continued sweep. -/
theorem eighteenth_payload_sweep_index_is_not_fortysix :
    (eighteenthPayloadContinueSweep 0).index ≠
      (eighteenthPayloadContinueSweepFromFortySix 0).index :=
  eighteenthPayloadContinueSweep_ne_fortysix 0

/-- This payload's sweep cursor is `FLAG`, not the first payload `1|FLAG`. -/
theorem eighteenth_payload_sweep_validator_is_not_first_payload :
    (eighteenthPayloadContinueSweep 0).validatorIndex ≠
      (firstPayloadSkipTakeBreakCapWithdrawal 0).validatorIndex :=
  eighteenthPayloadContinueSweep_ne_first_payload_validator 0

/-- Mutant: reuse the seventeenth payload's second-sweep `1|FLAG`. -/
theorem eighteenth_payload_sweep_validator_is_not_seventeenth_second :
    (eighteenthPayloadContinueSweep 0).validatorIndex ≠
      (seventeenthPayloadContinueSweepSecond 0).validatorIndex :=
  eighteenthPayloadContinueSweep_ne_seventeenth_second_validator 0

/-- Capella:458 / Gloas:1868. The eighteenth payload's second sweep is
`start+48`, not frozen at `start+47`. -/
theorem eighteenth_payload_sweep_second_index_is_start_plus_48 :
    (eighteenthPayloadContinueSweepSecond 0).index = 48 :=
  eighteenthPayloadContinueSweepSecond_index 0

theorem eighteenth_payload_sweep_second_is_not_frozen :
    (eighteenthPayloadContinueSweepSecond 0).index ≠
      (eighteenthPayloadContinueSweepSecondFrozen 0).index :=
  eighteenthPayloadContinueSweepSecond_ne_frozen 0

theorem eighteenth_payload_sweep_second_is_not_this_first :
    (eighteenthPayloadContinueSweepSecond 0).index ≠
      (eighteenthPayloadContinueSweep 0).index :=
  eighteenthPayloadContinueSweepSecond_ne_first 0

/-- Validator is `1|FLAG`, not this payload's first `FLAG`. -/
theorem eighteenth_payload_sweep_second_validator_is_not_this_first :
    (eighteenthPayloadContinueSweepSecond 0).validatorIndex ≠
      (eighteenthPayloadContinueSweep 0).validatorIndex :=
  eighteenthPayloadContinueSweepSecond_ne_first_validator 0

/-- Validator is `1|FLAG`, not raw builder 1. -/
theorem eighteenth_payload_sweep_second_validator_is_not_raw :
    (eighteenthPayloadContinueSweepSecond 0).validatorIndex ≠ 1 :=
  eighteenthPayloadContinueSweepSecond_ne_raw 0

/-- Frozen continued pair of the eighteenth payload repeats `start+47`. -/
theorem eighteenth_payload_continued_pair_is_not_frozen :
    (firstPayloadTwoExitedWithdrawals 47).map (fun w => w.index) ≠
      (firstPayloadTwoExitedWithdrawalsFrozenIndex 47).map (fun w => w.index) :=
  eighteenthPayloadContinueSweep_pair_ne_frozen 0

/-- Capella:510 then 458. The eighteen-payload chain is `indexSeq start 49`,
not the 47-item omit. -/
theorem eighteen_payload_fortynine_is_not_omit :
    indexSeq 0 49 ≠ indexSeq 0 47 := by
  intro h
  have := congrArg List.length h
  simp [indexSeq_length] at this

/-- Capella:506-510. After 49 appends the cursor is 49, not 47 or 48. -/
theorem eighteen_payload_cursor_is_not_omit :
    updateNextWithdrawalIndex 0 (indexSeq 0 49) ≠ 47 := by
  rw [updateNextWithdrawalIndex_seq (by decide : 0 < 49)]
  decide

theorem eighteen_payload_cursor_is_not_frozen :
    updateNextWithdrawalIndex 0 (indexSeq 0 49) ≠ 48 := by
  rw [updateNextWithdrawalIndex_seq (by decide : 0 < 49)]
  decide

/-- Restarting the eighteenth pair at 0 repeats indices 0 and 1. -/
theorem eighteen_payload_restart_repeats_prefix :
    indexSeq 0 47 ++ indexSeq 0 2 ≠ indexSeq 0 49 := by
  intro h
  have hsplit :
      indexSeq 0 47 ++ indexSeq 0 2 =
        indexSeq 0 47 ++ indexSeq 47 2 := by
    rw [← indexSeq_append 0 47 2] at h
    exact h
  have hcancel := List.append_cancel_left hsplit
  exact (by decide : 0 ≠ 47) (indexSeq_start_inj (by decide : 0 < 2) hcancel)

/-- fork.py:1118. The 49-chain is queues plus 211 Gwei, not the
47-chain's queues plus 199 Gwei. -/
theorem fortynine_chain_credits_are_not_fortyseven :
    211 * GWEI_TO_WEI ≠ 199 * GWEI_TO_WEI := by
  simp [GWEI_TO_WEI]

/-- fork.py:1118. The 49-chain is not the 47-chain credit
(`items b1 + 192e9`). -/
theorem fortynine_chain_credits_are_not_sixteen_pairs :
    204 * GWEI_TO_WEI ≠ 192 * GWEI_TO_WEI := by
  simp [GWEI_TO_WEI]
...
/-- Capella:458 / Gloas:1868. The twentieth payload's second sweep is
`start+52`, not frozen at `start+51`. -/
theorem twentieth_payload_sweep_second_index_is_start_plus_52 :
    (twentiethPayloadContinueSweepSecond 0).index = 52 :=
  twentiethPayloadContinueSweepSecond_index 0

theorem twentieth_payload_sweep_second_is_not_frozen :
    (twentiethPayloadContinueSweepSecond 0).index ≠
      (twentiethPayloadContinueSweepSecondFrozen 0).index :=
  twentiethPayloadContinueSweepSecond_ne_frozen 0

theorem twentieth_payload_sweep_second_is_not_this_first :
    (twentiethPayloadContinueSweepSecond 0).index ≠
      (twentiethPayloadContinueSweep 0).index :=
  twentiethPayloadContinueSweepSecond_ne_first 0

/-- Validator is `1|FLAG`, not this payload's first `FLAG`. -/
theorem twentieth_payload_sweep_second_validator_is_not_this_first :
    (twentiethPayloadContinueSweepSecond 0).validatorIndex ≠
      (twentiethPayloadContinueSweep 0).validatorIndex :=
  twentiethPayloadContinueSweepSecond_ne_first_validator 0

/-- Validator is `1|FLAG`, not raw builder 1. -/
theorem twentieth_payload_sweep_second_validator_is_not_raw :
    (twentiethPayloadContinueSweepSecond 0).validatorIndex ≠ 1 :=
  twentiethPayloadContinueSweepSecond_ne_raw 0

/-- Frozen continued pair of the twentieth payload repeats `start+51`. -/
theorem twentieth_payload_continued_pair_is_not_frozen :
    (firstPayloadTwoExitedWithdrawals 51).map (fun w => w.index) ≠
      (firstPayloadTwoExitedWithdrawalsFrozenIndex 51).map (fun w => w.index) :=
  twentiethPayloadContinueSweep_pair_ne_frozen 0

/-- Capella:510 then 458. The twenty-payload chain is `indexSeq start 53`,
not the 51-item omit. -/
theorem twenty_payload_fiftythree_is_not_omit :
    indexSeq 0 53 ≠ indexSeq 0 51 := by
  intro h
  have := congrArg List.length h
  simp [indexSeq_length] at this

/-- Capella:506-510. After 53 appends the cursor is 53, not 51 or 52. -/
theorem twenty_payload_cursor_is_not_omit :
    updateNextWithdrawalIndex 0 (indexSeq 0 53) ≠ 51 := by
  rw [updateNextWithdrawalIndex_seq (by decide : 0 < 53)]
  decide

theorem twenty_payload_cursor_is_not_frozen :
    updateNextWithdrawalIndex 0 (indexSeq 0 53) ≠ 52 := by
  rw [updateNextWithdrawalIndex_seq (by decide : 0 < 53)]
  decide

/-- Restarting the twentieth pair at 0 repeats indices 0 and 1. -/
theorem twenty_payload_restart_repeats_prefix :
    indexSeq 0 51 ++ indexSeq 0 2 ≠ indexSeq 0 53 := by
  intro h
  have hsplit :
      indexSeq 0 51 ++ indexSeq 0 2 =
        indexSeq 0 51 ++ indexSeq 51 2 := by
    rw [← indexSeq_append 0 51 2] at h
    exact h
  have hcancel := List.append_cancel_left hsplit
  exact (by decide : 0 ≠ 51) (indexSeq_start_inj (by decide : 0 < 2) hcancel)

/-- fork.py:1118. The 53-chain is queues plus 235 Gwei, not the
51-chain's queues plus 223 Gwei. -/
theorem fiftythree_chain_credits_are_not_fiftyone :
    235 * GWEI_TO_WEI ≠ 223 * GWEI_TO_WEI := by
  simp [GWEI_TO_WEI]

/-- fork.py:1118. The 53-chain is not the 51-chain credit
(`items b1 + 216e9`). -/
theorem fiftythree_chain_credits_are_not_eighteen_pairs :
    228 * GWEI_TO_WEI ≠ 216 * GWEI_TO_WEI := by
  simp [GWEI_TO_WEI]



/-- Capella:452/480. The twenty-first payload's first sweep is stamped at
`start+53`, not restarted at 0. -/
theorem twenty_first_payload_sweep_index_is_not_restart :
    (twentyFirstPayloadContinueSweep 0).index ≠
      (twentyFirstPayloadContinueSweepRestart 0).index :=
  twentyFirstPayloadContinueSweep_ne_restart 0

/-- Mutant: stamp from Gloas:2016 visits=2. -/
theorem twenty_first_payload_sweep_index_is_not_visits :
    (twentyFirstPayloadContinueSweep 0).index ≠
      (twentyFirstPayloadContinueSweepFromVisits 0).index :=
  twentyFirstPayloadContinueSweep_ne_visits 0

/-- Mutant: stamp from the 51-chain cursor. -/
theorem twenty_first_payload_sweep_index_is_not_fiftyone :
    (twentyFirstPayloadContinueSweep 0).index ≠
      (twentyFirstPayloadContinueSweepFromFiftyOne 0).index :=
  twentyFirstPayloadContinueSweep_ne_fiftyone 0

/-- Mutant: freeze after the twentieth payload's second continued sweep. -/
theorem twenty_first_payload_sweep_index_is_not_fiftytwo :
    (twentyFirstPayloadContinueSweep 0).index ≠
      (twentyFirstPayloadContinueSweepFromFiftyTwo 0).index :=
  twentyFirstPayloadContinueSweep_ne_fiftytwo 0

/-- This payload's sweep cursor is `FLAG`, not the first payload `1|FLAG`. -/
theorem twenty_first_payload_sweep_validator_is_not_first_payload :
    (twentyFirstPayloadContinueSweep 0).validatorIndex ≠
      (firstPayloadSkipTakeBreakCapWithdrawal 0).validatorIndex :=
  twentyFirstPayloadContinueSweep_ne_first_payload_validator 0

/-- Mutant: reuse the twentieth payload's second-sweep `1|FLAG`. -/
theorem twenty_first_payload_sweep_validator_is_not_twentieth_second :
    (twentyFirstPayloadContinueSweep 0).validatorIndex ≠
      (twentiethPayloadContinueSweepSecond 0).validatorIndex :=
  twentyFirstPayloadContinueSweep_ne_twentieth_second_validator 0

/-- Capella:458 / Gloas:1868. The twenty-first payload's second sweep is
`start+54`, not frozen at `start+53`. -/
theorem twenty_first_payload_sweep_second_index_is_start_plus_54 :
    (twentyFirstPayloadContinueSweepSecond 0).index = 54 :=
  twentyFirstPayloadContinueSweepSecond_index 0

theorem twenty_first_payload_sweep_second_is_not_frozen :
    (twentyFirstPayloadContinueSweepSecond 0).index ≠
      (twentyFirstPayloadContinueSweepSecondFrozen 0).index :=
  twentyFirstPayloadContinueSweepSecond_ne_frozen 0

theorem twenty_first_payload_sweep_second_is_not_this_first :
    (twentyFirstPayloadContinueSweepSecond 0).index ≠
      (twentyFirstPayloadContinueSweep 0).index :=
  twentyFirstPayloadContinueSweepSecond_ne_first 0

/-- Validator is `1|FLAG`, not this payload's first `FLAG`. -/
theorem twenty_first_payload_sweep_second_validator_is_not_this_first :
    (twentyFirstPayloadContinueSweepSecond 0).validatorIndex ≠
      (twentyFirstPayloadContinueSweep 0).validatorIndex :=
  twentyFirstPayloadContinueSweepSecond_ne_first_validator 0

/-- Validator is `1|FLAG`, not raw builder 1. -/
theorem twenty_first_payload_sweep_second_validator_is_not_raw :
    (twentyFirstPayloadContinueSweepSecond 0).validatorIndex ≠ 1 :=
  twentyFirstPayloadContinueSweepSecond_ne_raw 0

/-- Frozen continued pair of the twenty-first payload repeats `start+53`. -/
theorem twenty_first_payload_continued_pair_is_not_frozen :
    (firstPayloadTwoExitedWithdrawals 53).map (fun w => w.index) ≠
      (firstPayloadTwoExitedWithdrawalsFrozenIndex 53).map (fun w => w.index) :=
  twentyFirstPayloadContinueSweep_pair_ne_frozen 0

/-- Capella:510 then 458. The twenty-one-payload chain is `indexSeq start 55`,
not the 53-item omit. -/
theorem twenty_one_payload_fiftyfive_is_not_omit :
    indexSeq 0 55 ≠ indexSeq 0 53 := by
  intro h
  have := congrArg List.length h
  simp [indexSeq_length] at this

/-- Capella:506-510. After 55 appends the cursor is 55, not 53 or 54. -/
theorem twenty_one_payload_cursor_is_not_omit :
    updateNextWithdrawalIndex 0 (indexSeq 0 55) ≠ 53 := by
  rw [updateNextWithdrawalIndex_seq (by decide : 0 < 55)]
  decide

theorem twenty_one_payload_cursor_is_not_frozen :
    updateNextWithdrawalIndex 0 (indexSeq 0 55) ≠ 54 := by
  rw [updateNextWithdrawalIndex_seq (by decide : 0 < 55)]
  decide

/-- Restarting the twenty-first pair at 0 repeats indices 0 and 1. -/
theorem twenty_one_payload_restart_repeats_prefix :
    indexSeq 0 53 ++ indexSeq 0 2 ≠ indexSeq 0 55 := by
  intro h
  have hsplit :
      indexSeq 0 53 ++ indexSeq 0 2 =
        indexSeq 0 53 ++ indexSeq 53 2 := by
    rw [← indexSeq_append 0 53 2] at h
    exact h
  have hcancel := List.append_cancel_left hsplit
  exact (by decide : 0 ≠ 53) (indexSeq_start_inj (by decide : 0 < 2) hcancel)

/-- fork.py:1118. The 55-chain is queues plus 247 Gwei, not the
53-chain's queues plus 235 Gwei. -/
theorem fiftyfive_chain_credits_are_not_fiftythree :
    247 * GWEI_TO_WEI ≠ 235 * GWEI_TO_WEI := by
  simp [GWEI_TO_WEI]

/-- fork.py:1118. The 55-chain is not the 53-chain credit
(`items b1 + 228e9`). -/
theorem fiftyfive_chain_credits_are_not_nineteen_pairs :
    240 * GWEI_TO_WEI ≠ 228 * GWEI_TO_WEI := by
  simp [GWEI_TO_WEI]



/-- Capella:452/480. The twenty-second payload's first sweep is stamped at
`start+55`, not restarted at 0. -/
theorem twenty_second_payload_sweep_index_is_not_restart :
    (twentySecondPayloadContinueSweep 0).index ≠
      (twentySecondPayloadContinueSweepRestart 0).index :=
  twentySecondPayloadContinueSweep_ne_restart 0

/-- Mutant: stamp from Gloas:2016 visits=2. -/
theorem twenty_second_payload_sweep_index_is_not_visits :
    (twentySecondPayloadContinueSweep 0).index ≠
      (twentySecondPayloadContinueSweepFromVisits 0).index :=
  twentySecondPayloadContinueSweep_ne_visits 0

/-- Mutant: stamp from the 53-chain cursor. -/
theorem twenty_second_payload_sweep_index_is_not_fiftythree :
    (twentySecondPayloadContinueSweep 0).index ≠
      (twentySecondPayloadContinueSweepFromFiftyThree 0).index :=
  twentySecondPayloadContinueSweep_ne_fiftythree 0

/-- Mutant: freeze after the twenty-first payload's second continued sweep. -/
theorem twenty_second_payload_sweep_index_is_not_fiftyfour :
    (twentySecondPayloadContinueSweep 0).index ≠
      (twentySecondPayloadContinueSweepFromFiftyFour 0).index :=
  twentySecondPayloadContinueSweep_ne_fiftyfour 0

/-- This payload's sweep cursor is `FLAG`, not the first payload `1|FLAG`. -/
theorem twenty_second_payload_sweep_validator_is_not_first_payload :
    (twentySecondPayloadContinueSweep 0).validatorIndex ≠
      (firstPayloadSkipTakeBreakCapWithdrawal 0).validatorIndex :=
  twentySecondPayloadContinueSweep_ne_first_payload_validator 0

/-- Mutant: reuse the twenty-first payload's second-sweep `1|FLAG`. -/
theorem twenty_second_payload_sweep_validator_is_not_twenty_first_second :
    (twentySecondPayloadContinueSweep 0).validatorIndex ≠
      (twentyFirstPayloadContinueSweepSecond 0).validatorIndex :=
  twentySecondPayloadContinueSweep_ne_twenty_first_second_validator 0

/-- Capella:458 / Gloas:1868. The twenty-second payload's second sweep is
`start+56`, not frozen at `start+55`. -/
theorem twenty_second_payload_sweep_second_index_is_start_plus_56 :
    (twentySecondPayloadContinueSweepSecond 0).index = 56 :=
  twentySecondPayloadContinueSweepSecond_index 0

theorem twenty_second_payload_sweep_second_is_not_frozen :
    (twentySecondPayloadContinueSweepSecond 0).index ≠
      (twentySecondPayloadContinueSweepSecondFrozen 0).index :=
  twentySecondPayloadContinueSweepSecond_ne_frozen 0

theorem twenty_second_payload_sweep_second_is_not_this_first :
    (twentySecondPayloadContinueSweepSecond 0).index ≠
      (twentySecondPayloadContinueSweep 0).index :=
  twentySecondPayloadContinueSweepSecond_ne_first 0

/-- Validator is `1|FLAG`, not this payload's first `FLAG`. -/
theorem twenty_second_payload_sweep_second_validator_is_not_this_first :
    (twentySecondPayloadContinueSweepSecond 0).validatorIndex ≠
      (twentySecondPayloadContinueSweep 0).validatorIndex :=
  twentySecondPayloadContinueSweepSecond_ne_first_validator 0

/-- Validator is `1|FLAG`, not raw builder 1. -/
theorem twenty_second_payload_sweep_second_validator_is_not_raw :
    (twentySecondPayloadContinueSweepSecond 0).validatorIndex ≠ 1 :=
  twentySecondPayloadContinueSweepSecond_ne_raw 0

/-- Frozen continued pair of the twenty-second payload repeats `start+55`. -/
theorem twenty_second_payload_continued_pair_is_not_frozen :
    (firstPayloadTwoExitedWithdrawals 55).map (fun w => w.index) ≠
      (firstPayloadTwoExitedWithdrawalsFrozenIndex 55).map (fun w => w.index) :=
  twentySecondPayloadContinueSweep_pair_ne_frozen 0

/-- Capella:510 then 458. The twenty-two-payload chain is `indexSeq start 57`,
not the 55-item omit. -/
theorem twenty_two_payload_fiftyseven_is_not_omit :
    indexSeq 0 57 ≠ indexSeq 0 55 := by
  intro h
  have := congrArg List.length h
  simp [indexSeq_length] at this

/-- Capella:506-510. After 57 appends the cursor is 57, not 55 or 56. -/
theorem twenty_two_payload_cursor_is_not_omit :
    updateNextWithdrawalIndex 0 (indexSeq 0 57) ≠ 55 := by
  rw [updateNextWithdrawalIndex_seq (by decide : 0 < 57)]
  decide

theorem twenty_two_payload_cursor_is_not_frozen :
    updateNextWithdrawalIndex 0 (indexSeq 0 57) ≠ 56 := by
  rw [updateNextWithdrawalIndex_seq (by decide : 0 < 57)]
  decide

/-- Restarting the twenty-second pair at 0 repeats indices 0 and 1. -/
theorem twenty_two_payload_restart_repeats_prefix :
    indexSeq 0 55 ++ indexSeq 0 2 ≠ indexSeq 0 57 := by
  intro h
  have hsplit :
      indexSeq 0 55 ++ indexSeq 0 2 =
        indexSeq 0 55 ++ indexSeq 55 2 := by
    rw [← indexSeq_append 0 55 2] at h
    exact h
  have hcancel := List.append_cancel_left hsplit
  exact (by decide : 0 ≠ 55) (indexSeq_start_inj (by decide : 0 < 2) hcancel)

/-- fork.py:1118. The 57-chain is queues plus 259 Gwei, not the
55-chain's queues plus 247 Gwei. -/
theorem fiftyseven_chain_credits_are_not_fiftyfive :
    259 * GWEI_TO_WEI ≠ 247 * GWEI_TO_WEI := by
  simp [GWEI_TO_WEI]

/-- fork.py:1118. The 57-chain is not the 55-chain credit
(`items b1 + 240e9`). -/
theorem fiftyseven_chain_credits_are_not_twenty_pairs :
    252 * GWEI_TO_WEI ≠ 240 * GWEI_TO_WEI := by
  simp [GWEI_TO_WEI]



/-- Capella:452/480. The twenty-third payload's first sweep is stamped at
`start+57`, not restarted at 0. -/
theorem twenty_third_payload_sweep_index_is_not_restart :
    (twentyThirdPayloadContinueSweep 0).index ≠
      (twentyThirdPayloadContinueSweepRestart 0).index :=
  twentyThirdPayloadContinueSweep_ne_restart 0

/-- Mutant: stamp from Gloas:2016 visits=2. -/
theorem twenty_third_payload_sweep_index_is_not_visits :
    (twentyThirdPayloadContinueSweep 0).index ≠
      (twentyThirdPayloadContinueSweepFromVisits 0).index :=
  twentyThirdPayloadContinueSweep_ne_visits 0

/-- Mutant: stamp from the 55-chain cursor. -/
theorem twenty_third_payload_sweep_index_is_not_fiftyfive :
    (twentyThirdPayloadContinueSweep 0).index ≠
      (twentyThirdPayloadContinueSweepFromFiftyFive 0).index :=
  twentyThirdPayloadContinueSweep_ne_fiftyfive 0

/-- Mutant: freeze after the twenty-second payload's second continued sweep. -/
theorem twenty_third_payload_sweep_index_is_not_fiftysix :
    (twentyThirdPayloadContinueSweep 0).index ≠
      (twentyThirdPayloadContinueSweepFromFiftySix 0).index :=
  twentyThirdPayloadContinueSweep_ne_fiftysix 0

/-- This payload's sweep cursor is `FLAG`, not the first payload `1|FLAG`. -/
theorem twenty_third_payload_sweep_validator_is_not_first_payload :
    (twentyThirdPayloadContinueSweep 0).validatorIndex ≠
      (firstPayloadSkipTakeBreakCapWithdrawal 0).validatorIndex :=
  twentyThirdPayloadContinueSweep_ne_first_payload_validator 0

/-- Mutant: reuse the twenty-second payload's second-sweep `1|FLAG`. -/
theorem twenty_third_payload_sweep_validator_is_not_twenty_second_second :
    (twentyThirdPayloadContinueSweep 0).validatorIndex ≠
      (twentySecondPayloadContinueSweepSecond 0).validatorIndex :=
  twentyThirdPayloadContinueSweep_ne_twenty_second_second_validator 0

/-- Capella:458 / Gloas:1868. The twenty-third payload's second sweep is
`start+58`, not frozen at `start+57`. -/
theorem twenty_third_payload_sweep_second_index_is_start_plus_58 :
    (twentyThirdPayloadContinueSweepSecond 0).index = 58 :=
  twentyThirdPayloadContinueSweepSecond_index 0

theorem twenty_third_payload_sweep_second_is_not_frozen :
    (twentyThirdPayloadContinueSweepSecond 0).index ≠
      (twentyThirdPayloadContinueSweepSecondFrozen 0).index :=
  twentyThirdPayloadContinueSweepSecond_ne_frozen 0

theorem twenty_third_payload_sweep_second_is_not_this_first :
    (twentyThirdPayloadContinueSweepSecond 0).index ≠
      (twentyThirdPayloadContinueSweep 0).index :=
  twentyThirdPayloadContinueSweepSecond_ne_first 0

/-- Validator is `1|FLAG`, not this payload's first `FLAG`. -/
theorem twenty_third_payload_sweep_second_validator_is_not_this_first :
    (twentyThirdPayloadContinueSweepSecond 0).validatorIndex ≠
      (twentyThirdPayloadContinueSweep 0).validatorIndex :=
  twentyThirdPayloadContinueSweepSecond_ne_first_validator 0

/-- Validator is `1|FLAG`, not raw builder 1. -/
theorem twenty_third_payload_sweep_second_validator_is_not_raw :
    (twentyThirdPayloadContinueSweepSecond 0).validatorIndex ≠ 1 :=
  twentyThirdPayloadContinueSweepSecond_ne_raw 0

/-- Frozen continued pair of the twenty-third payload repeats `start+57`. -/
theorem twenty_third_payload_continued_pair_is_not_frozen :
    (firstPayloadTwoExitedWithdrawals 57).map (fun w => w.index) ≠
      (firstPayloadTwoExitedWithdrawalsFrozenIndex 57).map (fun w => w.index) :=
  twentyThirdPayloadContinueSweep_pair_ne_frozen 0

/-- Capella:510 then 458. The twenty-three-payload chain is `indexSeq start 59`,
not the 57-item omit. -/
theorem twenty_three_payload_fiftynine_is_not_omit :
    indexSeq 0 59 ≠ indexSeq 0 57 := by
  intro h
  have := congrArg List.length h
  simp [indexSeq_length] at this

/-- Capella:506-510. After 59 appends the cursor is 59, not 57 or 58. -/
theorem twenty_three_payload_cursor_is_not_omit :
    updateNextWithdrawalIndex 0 (indexSeq 0 59) ≠ 57 := by
  rw [updateNextWithdrawalIndex_seq (by decide : 0 < 59)]
  decide

theorem twenty_three_payload_cursor_is_not_frozen :
    updateNextWithdrawalIndex 0 (indexSeq 0 59) ≠ 58 := by
  rw [updateNextWithdrawalIndex_seq (by decide : 0 < 59)]
  decide

/-- Restarting the twenty-third pair at 0 repeats indices 0 and 1. -/
theorem twenty_three_payload_restart_repeats_prefix :
    indexSeq 0 57 ++ indexSeq 0 2 ≠ indexSeq 0 59 := by
  intro h
  have hsplit :
      indexSeq 0 57 ++ indexSeq 0 2 =
        indexSeq 0 57 ++ indexSeq 57 2 := by
    rw [← indexSeq_append 0 57 2] at h
    exact h
  have hcancel := List.append_cancel_left hsplit
  exact (by decide : 0 ≠ 57) (indexSeq_start_inj (by decide : 0 < 2) hcancel)

/-- fork.py:1118. The 59-chain is queues plus 271 Gwei, not the
57-chain's queues plus 259 Gwei. -/
theorem fiftynine_chain_credits_are_not_fiftyseven :
    271 * GWEI_TO_WEI ≠ 259 * GWEI_TO_WEI := by
  simp [GWEI_TO_WEI]

/-- fork.py:1118. The 59-chain is not the 57-chain credit
(`items b1 + 252e9`). -/
theorem fiftynine_chain_credits_are_not_twenty_pairs :
    264 * GWEI_TO_WEI ≠ 252 * GWEI_TO_WEI := by
  simp [GWEI_TO_WEI]
...
/-- Capella:458 / Gloas:1868. The twenty-fourth payload's second sweep is
`start+60`, not frozen at `start+59`. -/
theorem twenty_fourth_payload_sweep_second_index_is_start_plus_60 :
    (twentyFourthPayloadContinueSweepSecond 0).index = 60 :=
  twentyFourthPayloadContinueSweepSecond_index 0

theorem twenty_fourth_payload_sweep_second_is_not_frozen :
    (twentyFourthPayloadContinueSweepSecond 0).index ≠
      (twentyFourthPayloadContinueSweepSecondFrozen 0).index :=
  twentyFourthPayloadContinueSweepSecond_ne_frozen 0

theorem twenty_fourth_payload_sweep_second_is_not_this_first :
    (twentyFourthPayloadContinueSweepSecond 0).index ≠
      (twentyFourthPayloadContinueSweep 0).index :=
  twentyFourthPayloadContinueSweepSecond_ne_first 0

/-- Validator is `1|FLAG`, not this payload's first `FLAG`. -/
theorem twenty_fourth_payload_sweep_second_validator_is_not_this_first :
    (twentyFourthPayloadContinueSweepSecond 0).validatorIndex ≠
      (twentyFourthPayloadContinueSweep 0).validatorIndex :=
  twentyFourthPayloadContinueSweepSecond_ne_first_validator 0

/-- Validator is `1|FLAG`, not raw builder 1. -/
theorem twenty_fourth_payload_sweep_second_validator_is_not_raw :
    (twentyFourthPayloadContinueSweepSecond 0).validatorIndex ≠ 1 :=
  twentyFourthPayloadContinueSweepSecond_ne_raw 0

/-- Frozen continued pair of the twenty-fourth payload repeats `start+59`. -/
theorem twenty_fourth_payload_continued_pair_is_not_frozen :
    (firstPayloadTwoExitedWithdrawals 59).map (fun w => w.index) ≠
      (firstPayloadTwoExitedWithdrawalsFrozenIndex 59).map (fun w => w.index) :=
  twentyFourthPayloadContinueSweep_pair_ne_frozen 0

/-- Capella:510 then 458. The twenty-four-payload chain is `indexSeq start 61`,
not the 59-item omit. -/
theorem twenty_four_payload_sixtyone_is_not_omit :
    indexSeq 0 61 ≠ indexSeq 0 59 := by
  intro h
  have := congrArg List.length h
  simp [indexSeq_length] at this

/-- Capella:506-510. After 61 appends the cursor is 61, not 59 or 60. -/
theorem twenty_four_payload_cursor_is_not_omit :
    updateNextWithdrawalIndex 0 (indexSeq 0 61) ≠ 59 := by
  rw [updateNextWithdrawalIndex_seq (by decide : 0 < 61)]
  decide

theorem twenty_four_payload_cursor_is_not_frozen :
    updateNextWithdrawalIndex 0 (indexSeq 0 61) ≠ 60 := by
  rw [updateNextWithdrawalIndex_seq (by decide : 0 < 61)]
  decide

/-- Restarting the twenty-fourth pair at 0 repeats indices 0 and 1. -/
theorem twenty_four_payload_restart_repeats_prefix :
    indexSeq 0 59 ++ indexSeq 0 2 ≠ indexSeq 0 61 := by
  intro h
  have hsplit :
      indexSeq 0 59 ++ indexSeq 0 2 =
        indexSeq 0 59 ++ indexSeq 59 2 := by
    rw [← indexSeq_append 0 59 2] at h
    exact h
  have hcancel := List.append_cancel_left hsplit
  exact (by decide : 0 ≠ 59) (indexSeq_start_inj (by decide : 0 < 2) hcancel)

/-- fork.py:1118. The 61-chain is queues plus 283 Gwei, not the
59-chain's queues plus 271 Gwei. -/
theorem sixtyone_chain_credits_are_not_fiftynine :
    283 * GWEI_TO_WEI ≠ 271 * GWEI_TO_WEI := by
  simp [GWEI_TO_WEI]

/-- fork.py:1118. The 61-chain is not the 59-chain credit
(`items b1 + 264e9`). -/
theorem sixtyone_chain_credits_are_not_twenty_two_pairs :
    276 * GWEI_TO_WEI ≠ 264 * GWEI_TO_WEI := by
  simp [GWEI_TO_WEI]



/-- Capella:452/480. The twenty-fifth payload's first sweep is stamped at
`start+61`, not restarted at 0. -/
theorem twenty_fifth_payload_sweep_index_is_not_restart :
    (twentyFifthPayloadContinueSweep 0).index ≠
      (twentyFifthPayloadContinueSweepRestart 0).index :=
  twentyFifthPayloadContinueSweep_ne_restart 0

/-- Mutant: stamp from Gloas:2016 visits=2. -/
theorem twenty_fifth_payload_sweep_index_is_not_visits :
    (twentyFifthPayloadContinueSweep 0).index ≠
      (twentyFifthPayloadContinueSweepFromVisits 0).index :=
  twentyFifthPayloadContinueSweep_ne_visits 0

/-- Mutant: stamp from the 59-chain cursor. -/
theorem twenty_fifth_payload_sweep_index_is_not_fiftynine :
    (twentyFifthPayloadContinueSweep 0).index ≠
      (twentyFifthPayloadContinueSweepFromFiftyNine 0).index :=
  twentyFifthPayloadContinueSweep_ne_fiftynine 0

/-- Mutant: freeze after the twenty-fourth payload's second continued sweep. -/
theorem twenty_fifth_payload_sweep_index_is_not_sixty :
    (twentyFifthPayloadContinueSweep 0).index ≠
      (twentyFifthPayloadContinueSweepFromSixty 0).index :=
  twentyFifthPayloadContinueSweep_ne_sixty 0

/-- This payload's sweep cursor is `FLAG`, not the first payload `1|FLAG`. -/
theorem twenty_fifth_payload_sweep_validator_is_not_first_payload :
    (twentyFifthPayloadContinueSweep 0).validatorIndex ≠
      (firstPayloadSkipTakeBreakCapWithdrawal 0).validatorIndex :=
  twentyFifthPayloadContinueSweep_ne_first_payload_validator 0

/-- Mutant: reuse the twenty-fourth payload's second-sweep `1|FLAG`. -/
theorem twenty_fifth_payload_sweep_validator_is_not_twenty_fourth_second :
    (twentyFifthPayloadContinueSweep 0).validatorIndex ≠
      (twentyFourthPayloadContinueSweepSecond 0).validatorIndex :=
  twentyFifthPayloadContinueSweep_ne_twenty_fourth_second_validator 0

/-- Capella:458 / Gloas:1868. The twenty-fifth payload's second sweep is
`start+62`, not frozen at `start+61`. -/
theorem twenty_fifth_payload_sweep_second_index_is_start_plus_62 :
    (twentyFifthPayloadContinueSweepSecond 0).index = 62 :=
  twentyFifthPayloadContinueSweepSecond_index 0

theorem twenty_fifth_payload_sweep_second_is_not_frozen :
    (twentyFifthPayloadContinueSweepSecond 0).index ≠
      (twentyFifthPayloadContinueSweepSecondFrozen 0).index :=
  twentyFifthPayloadContinueSweepSecond_ne_frozen 0

theorem twenty_fifth_payload_sweep_second_is_not_this_first :
    (twentyFifthPayloadContinueSweepSecond 0).index ≠
      (twentyFifthPayloadContinueSweep 0).index :=
  twentyFifthPayloadContinueSweepSecond_ne_first 0

/-- Validator is `1|FLAG`, not this payload's first `FLAG`. -/
theorem twenty_fifth_payload_sweep_second_validator_is_not_this_first :
    (twentyFifthPayloadContinueSweepSecond 0).validatorIndex ≠
      (twentyFifthPayloadContinueSweep 0).validatorIndex :=
  twentyFifthPayloadContinueSweepSecond_ne_first_validator 0

/-- Validator is `1|FLAG`, not raw builder 1. -/
theorem twenty_fifth_payload_sweep_second_validator_is_not_raw :
    (twentyFifthPayloadContinueSweepSecond 0).validatorIndex ≠ 1 :=
  twentyFifthPayloadContinueSweepSecond_ne_raw 0

/-- Frozen continued pair of the twenty-fifth payload repeats `start+61`. -/
theorem twenty_fifth_payload_continued_pair_is_not_frozen :
    (firstPayloadTwoExitedWithdrawals 61).map (fun w => w.index) ≠
      (firstPayloadTwoExitedWithdrawalsFrozenIndex 61).map (fun w => w.index) :=
  twentyFifthPayloadContinueSweep_pair_ne_frozen 0

/-- Capella:510 then 458. The twenty-five-payload chain is `indexSeq start 63`,
not the 61-item omit. -/
theorem twenty_five_payload_sixtythree_is_not_omit :
    indexSeq 0 63 ≠ indexSeq 0 61 := by
  intro h
  have := congrArg List.length h
  simp [indexSeq_length] at this

/-- Capella:506-510. After 63 appends the cursor is 63, not 61 or 62. -/
theorem twenty_five_payload_cursor_is_not_omit :
    updateNextWithdrawalIndex 0 (indexSeq 0 63) ≠ 61 := by
  rw [updateNextWithdrawalIndex_seq (by decide : 0 < 63)]
  decide

theorem twenty_five_payload_cursor_is_not_frozen :
    updateNextWithdrawalIndex 0 (indexSeq 0 63) ≠ 62 := by
  rw [updateNextWithdrawalIndex_seq (by decide : 0 < 63)]
  decide

/-- Restarting the twenty-fifth pair at 0 repeats indices 0 and 1. -/
theorem twenty_five_payload_restart_repeats_prefix :
    indexSeq 0 61 ++ indexSeq 0 2 ≠ indexSeq 0 63 := by
  intro h
  have hsplit :
      indexSeq 0 61 ++ indexSeq 0 2 =
        indexSeq 0 61 ++ indexSeq 61 2 := by
    rw [← indexSeq_append 0 61 2] at h
    exact h
  have hcancel := List.append_cancel_left hsplit
  exact (by decide : 0 ≠ 61) (indexSeq_start_inj (by decide : 0 < 2) hcancel)

/-- fork.py:1118. The 63-chain is queues plus 295 Gwei, not the
61-chain's queues plus 283 Gwei. -/
theorem sixtythree_chain_credits_are_not_sixtyone :
    295 * GWEI_TO_WEI ≠ 283 * GWEI_TO_WEI := by
  simp [GWEI_TO_WEI]

/-- fork.py:1118. The 63-chain is not the 61-chain credit
(`items b1 + 276e9`). -/
theorem sixtythree_chain_credits_are_not_twenty_three_pairs :
    288 * GWEI_TO_WEI ≠ 276 * GWEI_TO_WEI := by
  simp [GWEI_TO_WEI]



/-- Capella:452/480. The twenty-sixth payload's first sweep is stamped at
`start+63`, not restarted at 0. -/
theorem twenty_sixth_payload_sweep_index_is_not_restart :
    (twentySixthPayloadContinueSweep 0).index ≠
      (twentySixthPayloadContinueSweepRestart 0).index :=
  twentySixthPayloadContinueSweep_ne_restart 0

/-- Mutant: stamp from Gloas:2016 visits=2. -/
theorem twenty_sixth_payload_sweep_index_is_not_visits :
    (twentySixthPayloadContinueSweep 0).index ≠
      (twentySixthPayloadContinueSweepFromVisits 0).index :=
  twentySixthPayloadContinueSweep_ne_visits 0

/-- Mutant: stamp from the 61-chain cursor. -/
theorem twenty_sixth_payload_sweep_index_is_not_sixtyone :
    (twentySixthPayloadContinueSweep 0).index ≠
      (twentySixthPayloadContinueSweepFromSixtyOne 0).index :=
  twentySixthPayloadContinueSweep_ne_sixtyone 0

/-- Mutant: freeze after the twenty-fifth payload's second continued sweep. -/
theorem twenty_sixth_payload_sweep_index_is_not_sixtytwo :
    (twentySixthPayloadContinueSweep 0).index ≠
      (twentySixthPayloadContinueSweepFromSixtyTwo 0).index :=
  twentySixthPayloadContinueSweep_ne_sixtytwo 0

/-- This payload's sweep cursor is `FLAG`, not the first payload `1|FLAG`. -/
theorem twenty_sixth_payload_sweep_validator_is_not_first_payload :
    (twentySixthPayloadContinueSweep 0).validatorIndex ≠
      (firstPayloadSkipTakeBreakCapWithdrawal 0).validatorIndex :=
  twentySixthPayloadContinueSweep_ne_first_payload_validator 0

/-- Mutant: reuse the twenty-fifth payload's second-sweep `1|FLAG`. -/
theorem twenty_sixth_payload_sweep_validator_is_not_twenty_fifth_second :
    (twentySixthPayloadContinueSweep 0).validatorIndex ≠
      (twentyFifthPayloadContinueSweepSecond 0).validatorIndex :=
  twentySixthPayloadContinueSweep_ne_twenty_fifth_second_validator 0

/-- Capella:458 / Gloas:1868. The twenty-sixth payload's second sweep is
`start+64`, not frozen at `start+63`. -/
theorem twenty_sixth_payload_sweep_second_index_is_start_plus_64 :
    (twentySixthPayloadContinueSweepSecond 0).index = 64 :=
  twentySixthPayloadContinueSweepSecond_index 0

theorem twenty_sixth_payload_sweep_second_is_not_frozen :
    (twentySixthPayloadContinueSweepSecond 0).index ≠
      (twentySixthPayloadContinueSweepSecondFrozen 0).index :=
  twentySixthPayloadContinueSweepSecond_ne_frozen 0

theorem twenty_sixth_payload_sweep_second_is_not_this_first :
    (twentySixthPayloadContinueSweepSecond 0).index ≠
      (twentySixthPayloadContinueSweep 0).index :=
  twentySixthPayloadContinueSweepSecond_ne_first 0

/-- Validator is `1|FLAG`, not this payload's first `FLAG`. -/
theorem twenty_sixth_payload_sweep_second_validator_is_not_this_first :
    (twentySixthPayloadContinueSweepSecond 0).validatorIndex ≠
      (twentySixthPayloadContinueSweep 0).validatorIndex :=
  twentySixthPayloadContinueSweepSecond_ne_first_validator 0

/-- Validator is `1|FLAG`, not raw builder 1. -/
theorem twenty_sixth_payload_sweep_second_validator_is_not_raw :
    (twentySixthPayloadContinueSweepSecond 0).validatorIndex ≠ 1 :=
  twentySixthPayloadContinueSweepSecond_ne_raw 0

/-- Frozen continued pair of the twenty-sixth payload repeats `start+63`. -/
theorem twenty_sixth_payload_continued_pair_is_not_frozen :
    (firstPayloadTwoExitedWithdrawals 63).map (fun w => w.index) ≠
      (firstPayloadTwoExitedWithdrawalsFrozenIndex 63).map (fun w => w.index) :=
  twentySixthPayloadContinueSweep_pair_ne_frozen 0

/-- Capella:510 then 458. The twenty-six-payload chain is `indexSeq start 65`,
not the 63-item omit. -/
theorem twenty_six_payload_sixtyfive_is_not_omit :
    indexSeq 0 65 ≠ indexSeq 0 63 := by
  intro h
  have := congrArg List.length h
  simp [indexSeq_length] at this

/-- Capella:506-510. After 65 appends the cursor is 65, not 63 or 64. -/
theorem twenty_six_payload_cursor_is_not_omit :
    updateNextWithdrawalIndex 0 (indexSeq 0 65) ≠ 63 := by
  rw [updateNextWithdrawalIndex_seq (by decide : 0 < 65)]
  decide

theorem twenty_six_payload_cursor_is_not_frozen :
    updateNextWithdrawalIndex 0 (indexSeq 0 65) ≠ 64 := by
  rw [updateNextWithdrawalIndex_seq (by decide : 0 < 65)]
  decide

/-- Restarting the twenty-sixth pair at 0 repeats indices 0 and 1. -/
theorem twenty_six_payload_restart_repeats_prefix :
    indexSeq 0 63 ++ indexSeq 0 2 ≠ indexSeq 0 65 := by
  intro h
  have hsplit :
      indexSeq 0 63 ++ indexSeq 0 2 =
        indexSeq 0 63 ++ indexSeq 63 2 := by
    rw [← indexSeq_append 0 63 2] at h
    exact h
  have hcancel := List.append_cancel_left hsplit
  exact (by decide : 0 ≠ 63) (indexSeq_start_inj (by decide : 0 < 2) hcancel)

/-- fork.py:1118. The 65-chain is queues plus 307 Gwei, not the
63-chain's queues plus 295 Gwei. -/
theorem sixtyfive_chain_credits_are_not_sixtythree :
    307 * GWEI_TO_WEI ≠ 295 * GWEI_TO_WEI := by
  simp [GWEI_TO_WEI]

/-- fork.py:1118. The 65-chain is not the 63-chain credit
(`items b1 + 288e9`). -/
theorem sixtyfive_chain_credits_are_not_twenty_four_pairs :
    300 * GWEI_TO_WEI ≠ 288 * GWEI_TO_WEI := by
  simp [GWEI_TO_WEI]



/-- Capella:452/480. The twenty-seventh payload's first sweep is stamped at
`start+65`, not restarted at 0. -/
theorem twenty_seventh_payload_sweep_index_is_not_restart :
    (twentySeventhPayloadContinueSweep 0).index ≠
      (twentySeventhPayloadContinueSweepRestart 0).index :=
  twentySeventhPayloadContinueSweep_ne_restart 0

/-- Mutant: stamp from Gloas:2016 visits=2. -/
theorem twenty_seventh_payload_sweep_index_is_not_visits :
    (twentySeventhPayloadContinueSweep 0).index ≠
      (twentySeventhPayloadContinueSweepFromVisits 0).index :=
  twentySeventhPayloadContinueSweep_ne_visits 0

/-- Mutant: stamp from the 63-chain cursor. -/
theorem twenty_seventh_payload_sweep_index_is_not_sixtythree :
    (twentySeventhPayloadContinueSweep 0).index ≠
      (twentySeventhPayloadContinueSweepFromSixtyThree 0).index :=
  twentySeventhPayloadContinueSweep_ne_sixtythree 0

/-- Mutant: freeze after the twenty-sixth payload's second continued sweep. -/
theorem twenty_seventh_payload_sweep_index_is_not_sixtyfour :
    (twentySeventhPayloadContinueSweep 0).index ≠
      (twentySeventhPayloadContinueSweepFromSixtyFour 0).index :=
  twentySeventhPayloadContinueSweep_ne_sixtyfour 0

/-- This payload's sweep cursor is `FLAG`, not the first payload `1|FLAG`. -/
theorem twenty_seventh_payload_sweep_validator_is_not_first_payload :
    (twentySeventhPayloadContinueSweep 0).validatorIndex ≠
      (firstPayloadSkipTakeBreakCapWithdrawal 0).validatorIndex :=
  twentySeventhPayloadContinueSweep_ne_first_payload_validator 0

/-- Mutant: reuse the twenty-fifth payload's second-sweep `1|FLAG`. -/
theorem twenty_seventh_payload_sweep_validator_is_not_twenty_sixth_second :
    (twentySeventhPayloadContinueSweep 0).validatorIndex ≠
      (twentySixthPayloadContinueSweepSecond 0).validatorIndex :=
  twentySeventhPayloadContinueSweep_ne_twenty_sixth_second_validator 0

/-- Capella:458 / Gloas:1868. The twenty-seventh payload's second sweep is
`start+66`, not frozen at `start+65`. -/
theorem twenty_seventh_payload_sweep_second_index_is_start_plus_66 :
    (twentySeventhPayloadContinueSweepSecond 0).index = 66 :=
  twentySeventhPayloadContinueSweepSecond_index 0

theorem twenty_seventh_payload_sweep_second_is_not_frozen :
    (twentySeventhPayloadContinueSweepSecond 0).index ≠
      (twentySeventhPayloadContinueSweepSecondFrozen 0).index :=
  twentySeventhPayloadContinueSweepSecond_ne_frozen 0

theorem twenty_seventh_payload_sweep_second_is_not_this_first :
    (twentySeventhPayloadContinueSweepSecond 0).index ≠
      (twentySeventhPayloadContinueSweep 0).index :=
  twentySeventhPayloadContinueSweepSecond_ne_first 0

/-- Validator is `1|FLAG`, not this payload's first `FLAG`. -/
theorem twenty_seventh_payload_sweep_second_validator_is_not_this_first :
    (twentySeventhPayloadContinueSweepSecond 0).validatorIndex ≠
      (twentySeventhPayloadContinueSweep 0).validatorIndex :=
  twentySeventhPayloadContinueSweepSecond_ne_first_validator 0

/-- Validator is `1|FLAG`, not raw builder 1. -/
theorem twenty_seventh_payload_sweep_second_validator_is_not_raw :
    (twentySeventhPayloadContinueSweepSecond 0).validatorIndex ≠ 1 :=
  twentySeventhPayloadContinueSweepSecond_ne_raw 0

/-- Frozen continued pair of the twenty-seventh payload repeats `start+65`. -/
theorem twenty_seventh_payload_continued_pair_is_not_frozen :
    (firstPayloadTwoExitedWithdrawals 65).map (fun w => w.index) ≠
      (firstPayloadTwoExitedWithdrawalsFrozenIndex 65).map (fun w => w.index) :=
  twentySeventhPayloadContinueSweep_pair_ne_frozen 0

/-- Capella:510 then 458. The twenty-seven-payload chain is `indexSeq start 67`,
not the 65-item omit. -/
theorem twenty_seven_payload_sixtyseven_is_not_omit :
    indexSeq 0 67 ≠ indexSeq 0 65 := by
  intro h
  have := congrArg List.length h
  simp [indexSeq_length] at this

/-- Capella:506-510. After 67 appends the cursor is 67, not 65 or 66. -/
theorem twenty_seven_payload_cursor_is_not_omit :
    updateNextWithdrawalIndex 0 (indexSeq 0 67) ≠ 65 := by
  rw [updateNextWithdrawalIndex_seq (by decide : 0 < 67)]
  decide

theorem twenty_seven_payload_cursor_is_not_frozen :
    updateNextWithdrawalIndex 0 (indexSeq 0 67) ≠ 66 := by
  rw [updateNextWithdrawalIndex_seq (by decide : 0 < 67)]
  decide

/-- Restarting the twenty-seventh pair at 0 repeats indices 0 and 1. -/
theorem twenty_seven_payload_restart_repeats_prefix :
    indexSeq 0 65 ++ indexSeq 0 2 ≠ indexSeq 0 67 := by
  intro h
  have hsplit :
      indexSeq 0 65 ++ indexSeq 0 2 =
        indexSeq 0 65 ++ indexSeq 65 2 := by
    rw [← indexSeq_append 0 65 2] at h
    exact h
  have hcancel := List.append_cancel_left hsplit
  exact (by decide : 0 ≠ 65) (indexSeq_start_inj (by decide : 0 < 2) hcancel)

/-- fork.py:1118. The 67-chain is queues plus 319 Gwei, not the
65-chain's queues plus 307 Gwei. -/
theorem sixtyseven_chain_credits_are_not_sixtyfive :
    319 * GWEI_TO_WEI ≠ 307 * GWEI_TO_WEI := by
  simp [GWEI_TO_WEI]

/-- fork.py:1118. The 67-chain is not the 65-chain credit
(`items b1 + 300e9`). -/
theorem sixtyseven_chain_credits_are_not_twenty_five_pairs :
    312 * GWEI_TO_WEI ≠ 300 * GWEI_TO_WEI := by
  simp [GWEI_TO_WEI]
...
/-- Capella:458 / Gloas:1868. The twenty-eighth payload's second sweep is
`start+68`, not frozen at `start+67`. -/
theorem twenty_eighth_payload_sweep_second_index_is_start_plus_68 :
    (twentyEighthPayloadContinueSweepSecond 0).index = 68 :=
  twentyEighthPayloadContinueSweepSecond_index 0

theorem twenty_eighth_payload_sweep_second_is_not_frozen :
    (twentyEighthPayloadContinueSweepSecond 0).index ≠
      (twentyEighthPayloadContinueSweepSecondFrozen 0).index :=
  twentyEighthPayloadContinueSweepSecond_ne_frozen 0

theorem twenty_eighth_payload_sweep_second_is_not_this_first :
    (twentyEighthPayloadContinueSweepSecond 0).index ≠
      (twentyEighthPayloadContinueSweep 0).index :=
  twentyEighthPayloadContinueSweepSecond_ne_first 0

/-- Validator is `1|FLAG`, not this payload's first `FLAG`. -/
theorem twenty_eighth_payload_sweep_second_validator_is_not_this_first :
    (twentyEighthPayloadContinueSweepSecond 0).validatorIndex ≠
      (twentyEighthPayloadContinueSweep 0).validatorIndex :=
  twentyEighthPayloadContinueSweepSecond_ne_first_validator 0

/-- Validator is `1|FLAG`, not raw builder 1. -/
theorem twenty_eighth_payload_sweep_second_validator_is_not_raw :
    (twentyEighthPayloadContinueSweepSecond 0).validatorIndex ≠ 1 :=
  twentyEighthPayloadContinueSweepSecond_ne_raw 0

/-- Frozen continued pair of the twenty-eighth payload repeats `start+67`. -/
theorem twenty_eighth_payload_continued_pair_is_not_frozen :
    (firstPayloadTwoExitedWithdrawals 67).map (fun w => w.index) ≠
      (firstPayloadTwoExitedWithdrawalsFrozenIndex 67).map (fun w => w.index) :=
  twentyEighthPayloadContinueSweep_pair_ne_frozen 0

/-- Capella:510 then 458. The twenty-eight-payload chain is `indexSeq start 69`,
not the 65-item omit. -/
theorem twenty_eight_payload_sixtynine_is_not_omit :
    indexSeq 0 69 ≠ indexSeq 0 67 := by
  intro h
  have := congrArg List.length h
  simp [indexSeq_length] at this

/-- Capella:506-510. After 69 appends the cursor is 69, not 67 or 68. -/
theorem twenty_eight_payload_cursor_is_not_omit :
    updateNextWithdrawalIndex 0 (indexSeq 0 69) ≠ 67 := by
  rw [updateNextWithdrawalIndex_seq (by decide : 0 < 69)]
  decide

theorem twenty_eight_payload_cursor_is_not_frozen :
    updateNextWithdrawalIndex 0 (indexSeq 0 69) ≠ 68 := by
  rw [updateNextWithdrawalIndex_seq (by decide : 0 < 69)]
  decide

/-- Restarting the twenty-eighth pair at 0 repeats indices 0 and 1. -/
theorem twenty_eight_payload_restart_repeats_prefix :
    indexSeq 0 67 ++ indexSeq 0 2 ≠ indexSeq 0 69 := by
  intro h
  have hsplit :
      indexSeq 0 67 ++ indexSeq 0 2 =
        indexSeq 0 67 ++ indexSeq 67 2 := by
    rw [← indexSeq_append 0 67 2] at h
    exact h
  have hcancel := List.append_cancel_left hsplit
  exact (by decide : 0 ≠ 67) (indexSeq_start_inj (by decide : 0 < 2) hcancel)

/-- fork.py:1118. The 69-chain is queues plus 331 Gwei, not the
67-chain's queues plus 319 Gwei. -/
theorem sixtynine_chain_credits_are_not_sixtyseven :
    331 * GWEI_TO_WEI ≠ 319 * GWEI_TO_WEI := by
  simp [GWEI_TO_WEI]

/-- fork.py:1118. The 69-chain is not the 67-chain credit
(`items b1 + 312e9`). -/
theorem sixtynine_chain_credits_are_not_twenty_six_pairs :
    324 * GWEI_TO_WEI ≠ 312 * GWEI_TO_WEI := by
  simp [GWEI_TO_WEI]



/-- Capella:452/480. The twenty-ninth payload's first sweep is stamped at
`start+69`, not restarted at 0. -/
theorem twenty_ninth_payload_sweep_index_is_not_restart :
    (twentyNinthPayloadContinueSweep 0).index ≠
      (twentyNinthPayloadContinueSweepRestart 0).index :=
  twentyNinthPayloadContinueSweep_ne_restart 0

/-- Mutant: stamp from Gloas:2016 visits=2. -/
theorem twenty_ninth_payload_sweep_index_is_not_visits :
    (twentyNinthPayloadContinueSweep 0).index ≠
      (twentyNinthPayloadContinueSweepFromVisits 0).index :=
  twentyNinthPayloadContinueSweep_ne_visits 0

/-- Mutant: stamp from the 67-chain cursor. -/
theorem twenty_ninth_payload_sweep_index_is_not_sixtyseven :
    (twentyNinthPayloadContinueSweep 0).index ≠
      (twentyNinthPayloadContinueSweepFromSixtySeven 0).index :=
  twentyNinthPayloadContinueSweep_ne_sixtyseven 0

/-- Mutant: freeze after the twenty-eighth payload's second continued sweep. -/
theorem twenty_ninth_payload_sweep_index_is_not_sixtyeight :
    (twentyNinthPayloadContinueSweep 0).index ≠
      (twentyNinthPayloadContinueSweepFromSixtyEight 0).index :=
  twentyNinthPayloadContinueSweep_ne_sixtyeight 0

/-- This payload's sweep cursor is `FLAG`, not the first payload `1|FLAG`. -/
theorem twenty_ninth_payload_sweep_validator_is_not_first_payload :
    (twentyNinthPayloadContinueSweep 0).validatorIndex ≠
      (firstPayloadSkipTakeBreakCapWithdrawal 0).validatorIndex :=
  twentyNinthPayloadContinueSweep_ne_first_payload_validator 0

/-- Mutant: reuse the twenty-eighth payload's second-sweep `1|FLAG`. -/
theorem twenty_ninth_payload_sweep_validator_is_not_twenty_eighth_second :
    (twentyNinthPayloadContinueSweep 0).validatorIndex ≠
      (twentyEighthPayloadContinueSweepSecond 0).validatorIndex :=
  twentyNinthPayloadContinueSweep_ne_twenty_eighth_second_validator 0

/-- Capella:458 / Gloas:1868. The twenty-ninth payload's second sweep is
`start+70`, not frozen at `start+69`. -/
theorem twenty_ninth_payload_sweep_second_index_is_start_plus_70 :
    (twentyNinthPayloadContinueSweepSecond 0).index = 70 :=
  twentyNinthPayloadContinueSweepSecond_index 0

theorem twenty_ninth_payload_sweep_second_is_not_frozen :
    (twentyNinthPayloadContinueSweepSecond 0).index ≠
      (twentyNinthPayloadContinueSweepSecondFrozen 0).index :=
  twentyNinthPayloadContinueSweepSecond_ne_frozen 0

theorem twenty_ninth_payload_sweep_second_is_not_this_first :
    (twentyNinthPayloadContinueSweepSecond 0).index ≠
      (twentyNinthPayloadContinueSweep 0).index :=
  twentyNinthPayloadContinueSweepSecond_ne_first 0

/-- Validator is `1|FLAG`, not this payload's first `FLAG`. -/
theorem twenty_ninth_payload_sweep_second_validator_is_not_this_first :
    (twentyNinthPayloadContinueSweepSecond 0).validatorIndex ≠
      (twentyNinthPayloadContinueSweep 0).validatorIndex :=
  twentyNinthPayloadContinueSweepSecond_ne_first_validator 0

/-- Validator is `1|FLAG`, not raw builder 1. -/
theorem twenty_ninth_payload_sweep_second_validator_is_not_raw :
    (twentyNinthPayloadContinueSweepSecond 0).validatorIndex ≠ 1 :=
  twentyNinthPayloadContinueSweepSecond_ne_raw 0

/-- Frozen continued pair of the twenty-ninth payload repeats `start+69`. -/
theorem twenty_ninth_payload_continued_pair_is_not_frozen :
    (firstPayloadTwoExitedWithdrawals 69).map (fun w => w.index) ≠
      (firstPayloadTwoExitedWithdrawalsFrozenIndex 69).map (fun w => w.index) :=
  twentyNinthPayloadContinueSweep_pair_ne_frozen 0

/-- Capella:510 then 458. The twenty-nine-payload chain is `indexSeq start 71`,
not the 65-item omit. -/
theorem twenty_nine_payload_seventyone_is_not_omit :
    indexSeq 0 71 ≠ indexSeq 0 69 := by
  intro h
  have := congrArg List.length h
  simp [indexSeq_length] at this

/-- Capella:506-510. After 71 appends the cursor is 71, not 69 or 70. -/
theorem twenty_nine_payload_cursor_is_not_omit :
    updateNextWithdrawalIndex 0 (indexSeq 0 71) ≠ 69 := by
  rw [updateNextWithdrawalIndex_seq (by decide : 0 < 71)]
  decide

theorem twenty_nine_payload_cursor_is_not_frozen :
    updateNextWithdrawalIndex 0 (indexSeq 0 71) ≠ 70 := by
  rw [updateNextWithdrawalIndex_seq (by decide : 0 < 71)]
  decide

/-- Restarting the twenty-ninth pair at 0 repeats indices 0 and 1. -/
theorem twenty_nine_payload_restart_repeats_prefix :
    indexSeq 0 69 ++ indexSeq 0 2 ≠ indexSeq 0 71 := by
  intro h
  have hsplit :
      indexSeq 0 69 ++ indexSeq 0 2 =
        indexSeq 0 69 ++ indexSeq 69 2 := by
    rw [← indexSeq_append 0 69 2] at h
    exact h
  have hcancel := List.append_cancel_left hsplit
  exact (by decide : 0 ≠ 69) (indexSeq_start_inj (by decide : 0 < 2) hcancel)

/-- fork.py:1118. The 71-chain is queues plus 343 Gwei, not the
69-chain's queues plus 331 Gwei. -/
theorem seventyone_chain_credits_are_not_sixtynine :
    343 * GWEI_TO_WEI ≠ 331 * GWEI_TO_WEI := by
  simp [GWEI_TO_WEI]

/-- fork.py:1118. The 71-chain is not the 69-chain credit
(`items b1 + 324e9`). -/
theorem seventyone_chain_credits_are_not_twenty_seven_pairs :
    336 * GWEI_TO_WEI ≠ 324 * GWEI_TO_WEI := by
  simp [GWEI_TO_WEI]
...
/-- Capella:458 / Gloas:1868. The thirtieth payload's second sweep is
`start+72`, not frozen at `start+71`. -/
theorem thirtieth_payload_sweep_second_index_is_start_plus_72 :
    (thirtiethPayloadContinueSweepSecond 0).index = 72 :=
  thirtiethPayloadContinueSweepSecond_index 0

theorem thirtieth_payload_sweep_second_is_not_frozen :
    (thirtiethPayloadContinueSweepSecond 0).index ≠
      (thirtiethPayloadContinueSweepSecondFrozen 0).index :=
  thirtiethPayloadContinueSweepSecond_ne_frozen 0

theorem thirtieth_payload_sweep_second_is_not_this_first :
    (thirtiethPayloadContinueSweepSecond 0).index ≠
      (thirtiethPayloadContinueSweep 0).index :=
  thirtiethPayloadContinueSweepSecond_ne_first 0

/-- Validator is `1|FLAG`, not this payload's first `FLAG`. -/
theorem thirtieth_payload_sweep_second_validator_is_not_this_first :
    (thirtiethPayloadContinueSweepSecond 0).validatorIndex ≠
      (thirtiethPayloadContinueSweep 0).validatorIndex :=
  thirtiethPayloadContinueSweepSecond_ne_first_validator 0

/-- Validator is `1|FLAG`, not raw builder 1. -/
theorem thirtieth_payload_sweep_second_validator_is_not_raw :
    (thirtiethPayloadContinueSweepSecond 0).validatorIndex ≠ 1 :=
  thirtiethPayloadContinueSweepSecond_ne_raw 0

/-- Frozen continued pair of the thirtieth payload repeats `start+71`. -/
theorem thirtieth_payload_continued_pair_is_not_frozen :
    (firstPayloadTwoExitedWithdrawals 71).map (fun w => w.index) ≠
      (firstPayloadTwoExitedWithdrawalsFrozenIndex 71).map (fun w => w.index) :=
  thirtiethPayloadContinueSweep_pair_ne_frozen 0

/-- Capella:510 then 458. The thirty-payload chain is `indexSeq start 73`,
not the 65-item omit. -/
theorem thirty_payload_seventythree_is_not_omit :
    indexSeq 0 73 ≠ indexSeq 0 71 := by
  intro h
  have := congrArg List.length h
  simp [indexSeq_length] at this

/-- Capella:506-510. After 73 appends the cursor is 73, not 71 or 72. -/
theorem thirty_payload_cursor_is_not_omit :
    updateNextWithdrawalIndex 0 (indexSeq 0 73) ≠ 71 := by
  rw [updateNextWithdrawalIndex_seq (by decide : 0 < 73)]
  decide

theorem thirty_payload_cursor_is_not_frozen :
    updateNextWithdrawalIndex 0 (indexSeq 0 73) ≠ 72 := by
  rw [updateNextWithdrawalIndex_seq (by decide : 0 < 73)]
  decide

/-- Restarting the thirtieth pair at 0 repeats indices 0 and 1. -/
theorem thirty_payload_restart_repeats_prefix :
    indexSeq 0 71 ++ indexSeq 0 2 ≠ indexSeq 0 73 := by
  intro h
  have hsplit :
      indexSeq 0 71 ++ indexSeq 0 2 =
        indexSeq 0 71 ++ indexSeq 71 2 := by
    rw [← indexSeq_append 0 71 2] at h
    exact h
  have hcancel := List.append_cancel_left hsplit
  exact (by decide : 0 ≠ 71) (indexSeq_start_inj (by decide : 0 < 2) hcancel)

/-- fork.py:1118. The 73-chain is queues plus 355 Gwei, not the
71-chain's queues plus 343 Gwei. -/
theorem seventythree_chain_credits_are_not_seventyone :
    355 * GWEI_TO_WEI ≠ 343 * GWEI_TO_WEI := by
  simp [GWEI_TO_WEI]

/-- fork.py:1118. The 73-chain is not the 71-chain credit
(`items b1 + 348e9`). -/
theorem seventythree_chain_credits_are_not_twenty_eight_pairs :
    348 * GWEI_TO_WEI ≠ 336 * GWEI_TO_WEI := by
  simp [GWEI_TO_WEI]
...
/-- Capella:458 / Gloas:1868. The thirty-first payload's second sweep is
`start+74`, not frozen at `start+73`. -/
theorem thirty_first_payload_sweep_second_index_is_start_plus_74 :
    (thirtyFirstPayloadContinueSweepSecond 0).index = 74 :=
  thirtyFirstPayloadContinueSweepSecond_index 0

theorem thirty_first_payload_sweep_second_is_not_frozen :
    (thirtyFirstPayloadContinueSweepSecond 0).index ≠
      (thirtyFirstPayloadContinueSweepSecondFrozen 0).index :=
  thirtyFirstPayloadContinueSweepSecond_ne_frozen 0

theorem thirty_first_payload_sweep_second_is_not_this_first :
    (thirtyFirstPayloadContinueSweepSecond 0).index ≠
      (thirtyFirstPayloadContinueSweep 0).index :=
  thirtyFirstPayloadContinueSweepSecond_ne_first 0

/-- Validator is `1|FLAG`, not this payload's first `FLAG`. -/
theorem thirty_first_payload_sweep_second_validator_is_not_this_first :
    (thirtyFirstPayloadContinueSweepSecond 0).validatorIndex ≠
      (thirtyFirstPayloadContinueSweep 0).validatorIndex :=
  thirtyFirstPayloadContinueSweepSecond_ne_first_validator 0

/-- Validator is `1|FLAG`, not raw builder 1. -/
theorem thirty_first_payload_sweep_second_validator_is_not_raw :
    (thirtyFirstPayloadContinueSweepSecond 0).validatorIndex ≠ 1 :=
  thirtyFirstPayloadContinueSweepSecond_ne_raw 0

/-- Frozen continued pair of the thirty-first payload repeats `start+73`. -/
theorem thirty_first_payload_continued_pair_is_not_frozen :
    (firstPayloadTwoExitedWithdrawals 73).map (fun w => w.index) ≠
      (firstPayloadTwoExitedWithdrawalsFrozenIndex 73).map (fun w => w.index) :=
  thirtyFirstPayloadContinueSweep_pair_ne_frozen 0

/-- Capella:510 then 458. The thirty-one-payload chain is `indexSeq start 75`,
not the 65-item omit. -/
theorem thirty_one_payload_seventyfive_is_not_omit :
    indexSeq 0 75 ≠ indexSeq 0 73 := by
  intro h
  have := congrArg List.length h
  simp [indexSeq_length] at this

/-- Capella:506-510. After 75 appends the cursor is 75, not 73 or 74. -/
theorem thirty_one_payload_cursor_is_not_omit :
    updateNextWithdrawalIndex 0 (indexSeq 0 75) ≠ 73 := by
  rw [updateNextWithdrawalIndex_seq (by decide : 0 < 75)]
  decide

theorem thirty_one_payload_cursor_is_not_frozen :
    updateNextWithdrawalIndex 0 (indexSeq 0 75) ≠ 74 := by
  rw [updateNextWithdrawalIndex_seq (by decide : 0 < 75)]
  decide

/-- Restarting the thirty-first pair at 0 repeats indices 0 and 1. -/
theorem thirty_one_payload_restart_repeats_prefix :
    indexSeq 0 73 ++ indexSeq 0 2 ≠ indexSeq 0 75 := by
  intro h
  have hsplit :
      indexSeq 0 73 ++ indexSeq 0 2 =
        indexSeq 0 73 ++ indexSeq 73 2 := by
    rw [← indexSeq_append 0 73 2] at h
    exact h
  have hcancel := List.append_cancel_left hsplit
  exact (by decide : 0 ≠ 73) (indexSeq_start_inj (by decide : 0 < 2) hcancel)

/-- fork.py:1118. The 75-chain is queues plus 367 Gwei, not the
73-chain's queues plus 355 Gwei. -/
theorem seventyfive_chain_credits_are_not_seventythree :
    367 * GWEI_TO_WEI ≠ 355 * GWEI_TO_WEI := by
  simp [GWEI_TO_WEI]

/-- fork.py:1118. The 75-chain is not the 73-chain credit
(`items b1 + 348e9`). -/
theorem seventyfive_chain_credits_are_not_twenty_nine_pairs :
    360 * GWEI_TO_WEI ≠ 348 * GWEI_TO_WEI := by
  simp [GWEI_TO_WEI]
...
/-- Capella:458 / Gloas:1868. The thirty-second payload's second sweep is
`start+76`, not frozen at `start+75`. -/
theorem thirty_second_payload_sweep_second_index_is_start_plus_76 :
    (thirtySecondPayloadContinueSweepSecond 0).index = 76 :=
  thirtySecondPayloadContinueSweepSecond_index 0

theorem thirty_second_payload_sweep_second_is_not_frozen :
    (thirtySecondPayloadContinueSweepSecond 0).index ≠
      (thirtySecondPayloadContinueSweepSecondFrozen 0).index :=
  thirtySecondPayloadContinueSweepSecond_ne_frozen 0

theorem thirty_second_payload_sweep_second_is_not_this_first :
    (thirtySecondPayloadContinueSweepSecond 0).index ≠
      (thirtySecondPayloadContinueSweep 0).index :=
  thirtySecondPayloadContinueSweepSecond_ne_first 0

/-- Validator is `1|FLAG`, not this payload's first `FLAG`. -/
theorem thirty_second_payload_sweep_second_validator_is_not_this_first :
    (thirtySecondPayloadContinueSweepSecond 0).validatorIndex ≠
      (thirtySecondPayloadContinueSweep 0).validatorIndex :=
  thirtySecondPayloadContinueSweepSecond_ne_first_validator 0

/-- Validator is `1|FLAG`, not raw builder 1. -/
theorem thirty_second_payload_sweep_second_validator_is_not_raw :
    (thirtySecondPayloadContinueSweepSecond 0).validatorIndex ≠ 1 :=
  thirtySecondPayloadContinueSweepSecond_ne_raw 0

/-- Frozen continued pair of the thirty-second payload repeats `start+75`. -/
theorem thirty_second_payload_continued_pair_is_not_frozen :
    (firstPayloadTwoExitedWithdrawals 75).map (fun w => w.index) ≠
      (firstPayloadTwoExitedWithdrawalsFrozenIndex 75).map (fun w => w.index) :=
  thirtySecondPayloadContinueSweep_pair_ne_frozen 0

/-- Capella:510 then 458. The thirty-two-payload chain is `indexSeq start 77`,
not the 65-item omit. -/
theorem thirty_two_payload_seventyseven_is_not_omit :
    indexSeq 0 77 ≠ indexSeq 0 75 := by
  intro h
  have := congrArg List.length h
  simp [indexSeq_length] at this

/-- Capella:506-510. After 77 appends the cursor is 77, not 75 or 76. -/
theorem thirty_two_payload_cursor_is_not_omit :
    updateNextWithdrawalIndex 0 (indexSeq 0 77) ≠ 75 := by
  rw [updateNextWithdrawalIndex_seq (by decide : 0 < 77)]
  decide

theorem thirty_two_payload_cursor_is_not_frozen :
    updateNextWithdrawalIndex 0 (indexSeq 0 77) ≠ 76 := by
  rw [updateNextWithdrawalIndex_seq (by decide : 0 < 77)]
  decide

/-- Restarting the thirty-second pair at 0 repeats indices 0 and 1. -/
theorem thirty_two_payload_restart_repeats_prefix :
    indexSeq 0 75 ++ indexSeq 0 2 ≠ indexSeq 0 77 := by
  intro h
  have hsplit :
      indexSeq 0 75 ++ indexSeq 0 2 =
        indexSeq 0 75 ++ indexSeq 75 2 := by
    rw [← indexSeq_append 0 75 2] at h
    exact h
  have hcancel := List.append_cancel_left hsplit
  exact (by decide : 0 ≠ 75) (indexSeq_start_inj (by decide : 0 < 2) hcancel)

/-- fork.py:1118. The 77-chain is queues plus 379 Gwei, not the
75-chain's queues plus 367 Gwei. -/
theorem seventyseven_chain_credits_are_not_seventyfive :
    379 * GWEI_TO_WEI ≠ 367 * GWEI_TO_WEI := by
  simp [GWEI_TO_WEI]

/-- fork.py:1118. The 77-chain is not the 75-chain credit
(`items b1 + 360e9`). -/
theorem seventyseven_chain_credits_are_not_thirty_pairs :
    372 * GWEI_TO_WEI ≠ 360 * GWEI_TO_WEI := by
  simp [GWEI_TO_WEI]



/-- Capella:452/480. The thirty-third payload's first sweep is stamped at
`start+77`, not restarted at 0. -/
theorem thirty_third_payload_sweep_index_is_not_restart :
    (thirtyThirdPayloadContinueSweep 0).index ≠
      (thirtyThirdPayloadContinueSweepRestart 0).index :=
  thirtyThirdPayloadContinueSweep_ne_restart 0

/-- Mutant: stamp from Gloas:2016 visits=2. -/
theorem thirty_third_payload_sweep_index_is_not_visits :
    (thirtyThirdPayloadContinueSweep 0).index ≠
      (thirtyThirdPayloadContinueSweepFromVisits 0).index :=
  thirtyThirdPayloadContinueSweep_ne_visits 0

/-- Mutant: stamp from the 75-chain cursor. -/
theorem thirty_third_payload_sweep_index_is_not_seventyfive :
    (thirtyThirdPayloadContinueSweep 0).index ≠
      (thirtyThirdPayloadContinueSweepFromSeventyFive 0).index :=
  thirtyThirdPayloadContinueSweep_ne_seventyfive 0

/-- Mutant: freeze after the thirty-second payload's second continued sweep. -/
theorem thirty_third_payload_sweep_index_is_not_seventysix :
    (thirtyThirdPayloadContinueSweep 0).index ≠
      (thirtyThirdPayloadContinueSweepFromSeventySix 0).index :=
  thirtyThirdPayloadContinueSweep_ne_seventysix 0

/-- This payload's sweep cursor is `FLAG`, not the first payload `1|FLAG`. -/
theorem thirty_third_payload_sweep_validator_is_not_first_payload :
    (thirtyThirdPayloadContinueSweep 0).validatorIndex ≠
      (firstPayloadSkipTakeBreakCapWithdrawal 0).validatorIndex :=
  thirtyThirdPayloadContinueSweep_ne_first_payload_validator 0

/-- Mutant: reuse the thirty-second payload's second-sweep `1|FLAG`. -/
theorem thirty_third_payload_sweep_validator_is_not_thirty_second_second :
    (thirtyThirdPayloadContinueSweep 0).validatorIndex ≠
      (thirtySecondPayloadContinueSweepSecond 0).validatorIndex :=
  thirtyThirdPayloadContinueSweep_ne_thirty_second_second_validator 0

/-- Capella:458 / Gloas:1868. The thirty-third payload's second sweep is
`start+78`, not frozen at `start+77`. -/
theorem thirty_third_payload_sweep_second_index_is_start_plus_78 :
    (thirtyThirdPayloadContinueSweepSecond 0).index = 78 :=
  thirtyThirdPayloadContinueSweepSecond_index 0

theorem thirty_third_payload_sweep_second_is_not_frozen :
    (thirtyThirdPayloadContinueSweepSecond 0).index ≠
      (thirtyThirdPayloadContinueSweepSecondFrozen 0).index :=
  thirtyThirdPayloadContinueSweepSecond_ne_frozen 0

theorem thirty_third_payload_sweep_second_is_not_this_first :
    (thirtyThirdPayloadContinueSweepSecond 0).index ≠
      (thirtyThirdPayloadContinueSweep 0).index :=
  thirtyThirdPayloadContinueSweepSecond_ne_first 0

/-- Validator is `1|FLAG`, not this payload's first `FLAG`. -/
theorem thirty_third_payload_sweep_second_validator_is_not_this_first :
    (thirtyThirdPayloadContinueSweepSecond 0).validatorIndex ≠
      (thirtyThirdPayloadContinueSweep 0).validatorIndex :=
  thirtyThirdPayloadContinueSweepSecond_ne_first_validator 0

/-- Validator is `1|FLAG`, not raw builder 1. -/
theorem thirty_third_payload_sweep_second_validator_is_not_raw :
    (thirtyThirdPayloadContinueSweepSecond 0).validatorIndex ≠ 1 :=
  thirtyThirdPayloadContinueSweepSecond_ne_raw 0

/-- Frozen continued pair of the thirty-third payload repeats `start+77`. -/
theorem thirty_third_payload_continued_pair_is_not_frozen :
    (firstPayloadTwoExitedWithdrawals 77).map (fun w => w.index) ≠
      (firstPayloadTwoExitedWithdrawalsFrozenIndex 77).map (fun w => w.index) :=
  thirtyThirdPayloadContinueSweep_pair_ne_frozen 0

/-- Capella:510 then 458. The thirty-three-payload chain is `indexSeq start 79`,
not the 65-item omit. -/
theorem thirty_three_payload_seventynine_is_not_omit :
    indexSeq 0 79 ≠ indexSeq 0 77 := by
  intro h
  have := congrArg List.length h
  simp [indexSeq_length] at this

/-- Capella:506-510. After 79 appends the cursor is 79, not 77 or 78. -/
theorem thirty_three_payload_cursor_is_not_omit :
    updateNextWithdrawalIndex 0 (indexSeq 0 79) ≠ 77 := by
  rw [updateNextWithdrawalIndex_seq (by decide : 0 < 79)]
  decide

theorem thirty_three_payload_cursor_is_not_frozen :
    updateNextWithdrawalIndex 0 (indexSeq 0 79) ≠ 78 := by
  rw [updateNextWithdrawalIndex_seq (by decide : 0 < 79)]
  decide

/-- Restarting the thirty-third pair at 0 repeats indices 0 and 1. -/
theorem thirty_three_payload_restart_repeats_prefix :
    indexSeq 0 77 ++ indexSeq 0 2 ≠ indexSeq 0 79 := by
  intro h
  have hsplit :
      indexSeq 0 77 ++ indexSeq 0 2 =
        indexSeq 0 77 ++ indexSeq 77 2 := by
    rw [← indexSeq_append 0 77 2] at h
    exact h
  have hcancel := List.append_cancel_left hsplit
  exact (by decide : 0 ≠ 77) (indexSeq_start_inj (by decide : 0 < 2) hcancel)

/-- fork.py:1118. The 79-chain is queues plus 391 Gwei, not the
77-chain's queues plus 379 Gwei. -/
theorem seventynine_chain_credits_are_not_seventyseven :
    391 * GWEI_TO_WEI ≠ 379 * GWEI_TO_WEI := by
  simp [GWEI_TO_WEI]

/-- fork.py:1118. The 79-chain is not the 77-chain credit
(`items b1 + 372e9`). -/
theorem seventynine_chain_credits_are_not_thirty_one_pairs :
    384 * GWEI_TO_WEI ≠ 372 * GWEI_TO_WEI := by
  simp [GWEI_TO_WEI]



/-- Capella:452/480. The thirty-fourth payload's first sweep is stamped at
`start+79`, not restarted at 0. -/
theorem thirty_fourth_payload_sweep_index_is_not_restart :
    (thirtyFourthPayloadContinueSweep 0).index ≠
      (thirtyFourthPayloadContinueSweepRestart 0).index :=
  thirtyFourthPayloadContinueSweep_ne_restart 0

/-- Mutant: stamp from Gloas:2016 visits=2. -/
theorem thirty_fourth_payload_sweep_index_is_not_visits :
    (thirtyFourthPayloadContinueSweep 0).index ≠
      (thirtyFourthPayloadContinueSweepFromVisits 0).index :=
  thirtyFourthPayloadContinueSweep_ne_visits 0

/-- Mutant: stamp from the 77-chain cursor. -/
theorem thirty_fourth_payload_sweep_index_is_not_seventyseven :
    (thirtyFourthPayloadContinueSweep 0).index ≠
      (thirtyFourthPayloadContinueSweepFromSeventySeven 0).index :=
  thirtyFourthPayloadContinueSweep_ne_seventyseven 0

/-- Mutant: freeze after the thirty-third payload's second continued sweep. -/
theorem thirty_fourth_payload_sweep_index_is_not_seventyeight :
    (thirtyFourthPayloadContinueSweep 0).index ≠
      (thirtyFourthPayloadContinueSweepFromSeventyEight 0).index :=
  thirtyFourthPayloadContinueSweep_ne_seventyeight 0

/-- This payload's sweep cursor is `FLAG`, not the first payload `1|FLAG`. -/
theorem thirty_fourth_payload_sweep_validator_is_not_first_payload :
    (thirtyFourthPayloadContinueSweep 0).validatorIndex ≠
      (firstPayloadSkipTakeBreakCapWithdrawal 0).validatorIndex :=
  thirtyFourthPayloadContinueSweep_ne_first_payload_validator 0

/-- Mutant: reuse the thirty-third payload's second-sweep `1|FLAG`. -/
theorem thirty_fourth_payload_sweep_validator_is_not_thirty_third_second :
    (thirtyFourthPayloadContinueSweep 0).validatorIndex ≠
      (thirtyThirdPayloadContinueSweepSecond 0).validatorIndex :=
  thirtyFourthPayloadContinueSweep_ne_thirty_third_second_validator 0

/-- Capella:458 / Gloas:1868. The thirty-fourth payload's second sweep is
`start+80`, not frozen at `start+79`. -/
theorem thirty_fourth_payload_sweep_second_index_is_start_plus_80 :
    (thirtyFourthPayloadContinueSweepSecond 0).index = 80 :=
  thirtyFourthPayloadContinueSweepSecond_index 0

theorem thirty_fourth_payload_sweep_second_is_not_frozen :
    (thirtyFourthPayloadContinueSweepSecond 0).index ≠
      (thirtyFourthPayloadContinueSweepSecondFrozen 0).index :=
  thirtyFourthPayloadContinueSweepSecond_ne_frozen 0

theorem thirty_fourth_payload_sweep_second_is_not_this_first :
    (thirtyFourthPayloadContinueSweepSecond 0).index ≠
      (thirtyFourthPayloadContinueSweep 0).index :=
  thirtyFourthPayloadContinueSweepSecond_ne_first 0

/-- Validator is `1|FLAG`, not this payload's first `FLAG`. -/
theorem thirty_fourth_payload_sweep_second_validator_is_not_this_first :
    (thirtyFourthPayloadContinueSweepSecond 0).validatorIndex ≠
      (thirtyFourthPayloadContinueSweep 0).validatorIndex :=
  thirtyFourthPayloadContinueSweepSecond_ne_first_validator 0

/-- Validator is `1|FLAG`, not raw builder 1. -/
theorem thirty_fourth_payload_sweep_second_validator_is_not_raw :
    (thirtyFourthPayloadContinueSweepSecond 0).validatorIndex ≠ 1 :=
  thirtyFourthPayloadContinueSweepSecond_ne_raw 0

/-- Frozen continued pair of the thirty-fourth payload repeats `start+79`. -/
theorem thirty_fourth_payload_continued_pair_is_not_frozen :
    (firstPayloadTwoExitedWithdrawals 79).map (fun w => w.index) ≠
      (firstPayloadTwoExitedWithdrawalsFrozenIndex 79).map (fun w => w.index) :=
  thirtyFourthPayloadContinueSweep_pair_ne_frozen 0

/-- Capella:510 then 458. The thirty-four-payload chain is `indexSeq start 81`,
not the 65-item omit. -/
theorem thirty_four_payload_eightyone_is_not_omit :
    indexSeq 0 81 ≠ indexSeq 0 79 := by
  intro h
  have := congrArg List.length h
  simp [indexSeq_length] at this

/-- Capella:506-510. After 81 appends the cursor is 81, not 79 or 80. -/
theorem thirty_four_payload_cursor_is_not_omit :
    updateNextWithdrawalIndex 0 (indexSeq 0 81) ≠ 79 := by
  rw [updateNextWithdrawalIndex_seq (by decide : 0 < 81)]
  decide

theorem thirty_four_payload_cursor_is_not_frozen :
    updateNextWithdrawalIndex 0 (indexSeq 0 81) ≠ 80 := by
  rw [updateNextWithdrawalIndex_seq (by decide : 0 < 81)]
  decide

/-- Restarting the thirty-fourth pair at 0 repeats indices 0 and 1. -/
theorem thirty_four_payload_restart_repeats_prefix :
    indexSeq 0 79 ++ indexSeq 0 2 ≠ indexSeq 0 81 := by
  intro h
  have hsplit :
      indexSeq 0 79 ++ indexSeq 0 2 =
        indexSeq 0 79 ++ indexSeq 79 2 := by
    rw [← indexSeq_append 0 79 2] at h
    exact h
  have hcancel := List.append_cancel_left hsplit
  exact (by decide : 0 ≠ 79) (indexSeq_start_inj (by decide : 0 < 2) hcancel)

/-- fork.py:1118. The 81-chain is queues plus 403 Gwei, not the
79-chain's queues plus 391 Gwei. -/
theorem eightyone_chain_credits_are_not_seventynine :
    403 * GWEI_TO_WEI ≠ 391 * GWEI_TO_WEI := by
  simp [GWEI_TO_WEI]

/-- fork.py:1118. The 81-chain is not the 79-chain credit
(`items b1 + 384e9`). -/
theorem eightyone_chain_credits_are_not_thirty_two_pairs :
    396 * GWEI_TO_WEI ≠ 384 * GWEI_TO_WEI := by
  simp [GWEI_TO_WEI]



/-- Capella:452/480. The thirty-fifth payload's first sweep is stamped at
`start+81`, not restarted at 0. -/
theorem thirty_fifth_payload_sweep_index_is_not_restart :
    (thirtyFifthPayloadContinueSweep 0).index ≠
      (thirtyFifthPayloadContinueSweepRestart 0).index :=
  thirtyFifthPayloadContinueSweep_ne_restart 0

/-- Mutant: stamp from Gloas:2016 visits=2. -/
theorem thirty_fifth_payload_sweep_index_is_not_visits :
    (thirtyFifthPayloadContinueSweep 0).index ≠
      (thirtyFifthPayloadContinueSweepFromVisits 0).index :=
  thirtyFifthPayloadContinueSweep_ne_visits 0

/-- Mutant: stamp from the 79-chain cursor. -/
theorem thirty_fifth_payload_sweep_index_is_not_seventynine :
    (thirtyFifthPayloadContinueSweep 0).index ≠
      (thirtyFifthPayloadContinueSweepFromSeventyNine 0).index :=
  thirtyFifthPayloadContinueSweep_ne_seventynine 0

/-- Mutant: freeze after the thirty-fourth payload's second continued sweep. -/
theorem thirty_fifth_payload_sweep_index_is_not_eighty :
    (thirtyFifthPayloadContinueSweep 0).index ≠
      (thirtyFifthPayloadContinueSweepFromEighty 0).index :=
  thirtyFifthPayloadContinueSweep_ne_eighty 0

/-- This payload's sweep cursor is `FLAG`, not the first payload `1|FLAG`. -/
theorem thirty_fifth_payload_sweep_validator_is_not_first_payload :
    (thirtyFifthPayloadContinueSweep 0).validatorIndex ≠
      (firstPayloadSkipTakeBreakCapWithdrawal 0).validatorIndex :=
  thirtyFifthPayloadContinueSweep_ne_first_payload_validator 0

/-- Mutant: reuse the thirty-fourth payload's second-sweep `1|FLAG`. -/
theorem thirty_fifth_payload_sweep_validator_is_not_thirty_fourth_second :
    (thirtyFifthPayloadContinueSweep 0).validatorIndex ≠
      (thirtyFourthPayloadContinueSweepSecond 0).validatorIndex :=
  thirtyFifthPayloadContinueSweep_ne_thirty_fourth_second_validator 0

/-- Capella:458 / Gloas:1868. The thirty-fifth payload's second sweep is
`start+82`, not frozen at `start+81`. -/
theorem thirty_fifth_payload_sweep_second_index_is_start_plus_82 :
    (thirtyFifthPayloadContinueSweepSecond 0).index = 82 :=
  thirtyFifthPayloadContinueSweepSecond_index 0

theorem thirty_fifth_payload_sweep_second_is_not_frozen :
    (thirtyFifthPayloadContinueSweepSecond 0).index ≠
      (thirtyFifthPayloadContinueSweepSecondFrozen 0).index :=
  thirtyFifthPayloadContinueSweepSecond_ne_frozen 0

theorem thirty_fifth_payload_sweep_second_is_not_this_first :
    (thirtyFifthPayloadContinueSweepSecond 0).index ≠
      (thirtyFifthPayloadContinueSweep 0).index :=
  thirtyFifthPayloadContinueSweepSecond_ne_first 0

/-- Validator is `1|FLAG`, not this payload's first `FLAG`. -/
theorem thirty_fifth_payload_sweep_second_validator_is_not_this_first :
    (thirtyFifthPayloadContinueSweepSecond 0).validatorIndex ≠
      (thirtyFifthPayloadContinueSweep 0).validatorIndex :=
  thirtyFifthPayloadContinueSweepSecond_ne_first_validator 0

/-- Validator is `1|FLAG`, not raw builder 1. -/
theorem thirty_fifth_payload_sweep_second_validator_is_not_raw :
    (thirtyFifthPayloadContinueSweepSecond 0).validatorIndex ≠ 1 :=
  thirtyFifthPayloadContinueSweepSecond_ne_raw 0

/-- Frozen continued pair of the thirty-fifth payload repeats `start+81`. -/
theorem thirty_fifth_payload_continued_pair_is_not_frozen :
    (firstPayloadTwoExitedWithdrawals 81).map (fun w => w.index) ≠
      (firstPayloadTwoExitedWithdrawalsFrozenIndex 81).map (fun w => w.index) :=
  thirtyFifthPayloadContinueSweep_pair_ne_frozen 0

/-- Capella:510 then 458. The thirty-five-payload chain is `indexSeq start 83`,
not the 65-item omit. -/
theorem thirty_five_payload_eightythree_is_not_omit :
    indexSeq 0 83 ≠ indexSeq 0 81 := by
  intro h
  have := congrArg List.length h
  simp [indexSeq_length] at this

/-- Capella:506-510. After 83 appends the cursor is 83, not 81 or 82. -/
theorem thirty_five_payload_cursor_is_not_omit :
    updateNextWithdrawalIndex 0 (indexSeq 0 83) ≠ 81 := by
  rw [updateNextWithdrawalIndex_seq (by decide : 0 < 83)]
  decide

theorem thirty_five_payload_cursor_is_not_frozen :
    updateNextWithdrawalIndex 0 (indexSeq 0 83) ≠ 82 := by
  rw [updateNextWithdrawalIndex_seq (by decide : 0 < 83)]
  decide

/-- Restarting the thirty-fifth pair at 0 repeats indices 0 and 1. -/
theorem thirty_five_payload_restart_repeats_prefix :
    indexSeq 0 81 ++ indexSeq 0 2 ≠ indexSeq 0 83 := by
  intro h
  have hsplit :
      indexSeq 0 81 ++ indexSeq 0 2 =
        indexSeq 0 81 ++ indexSeq 81 2 := by
    rw [← indexSeq_append 0 81 2] at h
    exact h
  have hcancel := List.append_cancel_left hsplit
  exact (by decide : 0 ≠ 81) (indexSeq_start_inj (by decide : 0 < 2) hcancel)

/-- fork.py:1118. The 83-chain is queues plus 415 Gwei, not the
81-chain's queues plus 403 Gwei. -/
theorem eightythree_chain_credits_are_not_eightyone :
    415 * GWEI_TO_WEI ≠ 403 * GWEI_TO_WEI := by
  simp [GWEI_TO_WEI]

/-- fork.py:1118. The 83-chain is not the 81-chain credit
(`items b1 + 396e9`). -/
theorem eightythree_chain_credits_are_not_thirty_three_pairs :
    408 * GWEI_TO_WEI ≠ 396 * GWEI_TO_WEI := by
  simp [GWEI_TO_WEI]
...
/-- Capella:452/480. The thirty-seventh payload's first sweep is stamped at
`start+85`, not restarted at 0. -/
theorem thirty_seventh_payload_sweep_index_is_not_restart :
    (thirtySeventhPayloadContinueSweep 0).index ≠
      (thirtySeventhPayloadContinueSweepRestart 0).index :=
  thirtySeventhPayloadContinueSweep_ne_restart 0

/-- Mutant: stamp from Gloas:2016 visits=2. -/
theorem thirty_seventh_payload_sweep_index_is_not_visits :
    (thirtySeventhPayloadContinueSweep 0).index ≠
      (thirtySeventhPayloadContinueSweepFromVisits 0).index :=
  thirtySeventhPayloadContinueSweep_ne_visits 0

/-- Mutant: stamp from the 83-chain cursor. -/
theorem thirty_seventh_payload_sweep_index_is_not_eightythree :
    (thirtySeventhPayloadContinueSweep 0).index ≠
      (thirtySeventhPayloadContinueSweepFromEightyThree 0).index :=
  thirtySeventhPayloadContinueSweep_ne_eightythree 0

/-- Mutant: freeze after the thirty-sixth payload's second continued sweep. -/
theorem thirty_seventh_payload_sweep_index_is_not_eightyfour :
    (thirtySeventhPayloadContinueSweep 0).index ≠
      (thirtySeventhPayloadContinueSweepFromEightyFour 0).index :=
  thirtySeventhPayloadContinueSweep_ne_eightyfour 0

/-- This payload's sweep cursor is `FLAG`, not the first payload `1|FLAG`. -/
theorem thirty_seventh_payload_sweep_validator_is_not_first_payload :
    (thirtySeventhPayloadContinueSweep 0).validatorIndex ≠
      (firstPayloadSkipTakeBreakCapWithdrawal 0).validatorIndex :=
  thirtySeventhPayloadContinueSweep_ne_first_payload_validator 0

/-- Mutant: reuse the thirty-sixth payload's second-sweep `1|FLAG`. -/
theorem thirty_seventh_payload_sweep_validator_is_not_thirty_sixth_second :
    (thirtySeventhPayloadContinueSweep 0).validatorIndex ≠
      (thirtySixthPayloadContinueSweepSecond 0).validatorIndex :=
  thirtySeventhPayloadContinueSweep_ne_thirty_sixth_second_validator 0

/-- Capella:458 / Gloas:1868. The thirty-seventh payload's second sweep is
`start+86`, not frozen at `start+85`. -/
theorem thirty_seventh_payload_sweep_second_index_is_start_plus_86 :
    (thirtySeventhPayloadContinueSweepSecond 0).index = 86 :=
  thirtySeventhPayloadContinueSweepSecond_index 0

theorem thirty_seventh_payload_sweep_second_is_not_frozen :
    (thirtySeventhPayloadContinueSweepSecond 0).index ≠
      (thirtySeventhPayloadContinueSweepSecondFrozen 0).index :=
  thirtySeventhPayloadContinueSweepSecond_ne_frozen 0

theorem thirty_seventh_payload_sweep_second_is_not_this_first :
    (thirtySeventhPayloadContinueSweepSecond 0).index ≠
      (thirtySeventhPayloadContinueSweep 0).index :=
  thirtySeventhPayloadContinueSweepSecond_ne_first 0

/-- Validator is `1|FLAG`, not this payload's first `FLAG`. -/
theorem thirty_seventh_payload_sweep_second_validator_is_not_this_first :
    (thirtySeventhPayloadContinueSweepSecond 0).validatorIndex ≠
      (thirtySeventhPayloadContinueSweep 0).validatorIndex :=
  thirtySeventhPayloadContinueSweepSecond_ne_first_validator 0

/-- Validator is `1|FLAG`, not raw builder 1. -/
theorem thirty_seventh_payload_sweep_second_validator_is_not_raw :
    (thirtySeventhPayloadContinueSweepSecond 0).validatorIndex ≠ 1 :=
  thirtySeventhPayloadContinueSweepSecond_ne_raw 0

/-- Frozen continued pair of the thirty-seventh payload repeats `start+85`. -/
theorem thirty_seventh_payload_continued_pair_is_not_frozen :
    (firstPayloadTwoExitedWithdrawals 85).map (fun w => w.index) ≠
      (firstPayloadTwoExitedWithdrawalsFrozenIndex 85).map (fun w => w.index) :=
  thirtySeventhPayloadContinueSweep_pair_ne_frozen 0

/-- Capella:510 then 458. The thirty-seven-payload chain is `indexSeq start 87`,
not the 65-item omit. -/
theorem thirty_seven_payload_eightyseven_is_not_omit :
    indexSeq 0 87 ≠ indexSeq 0 85 := by
  intro h
  have := congrArg List.length h
  simp [indexSeq_length] at this

/-- Capella:506-510. After 87 appends the cursor is 87, not 85 or 86. -/
theorem thirty_seven_payload_cursor_is_not_omit :
    updateNextWithdrawalIndex 0 (indexSeq 0 87) ≠ 85 := by
  rw [updateNextWithdrawalIndex_seq (by decide : 0 < 87)]
  decide

theorem thirty_seven_payload_cursor_is_not_frozen :
    updateNextWithdrawalIndex 0 (indexSeq 0 87) ≠ 86 := by
  rw [updateNextWithdrawalIndex_seq (by decide : 0 < 87)]
  decide

/-- Restarting the thirty-seventh pair at 0 repeats indices 0 and 1. -/
theorem thirty_seven_payload_restart_repeats_prefix :
    indexSeq 0 85 ++ indexSeq 0 2 ≠ indexSeq 0 87 := by
  intro h
  have hsplit :
      indexSeq 0 85 ++ indexSeq 0 2 =
        indexSeq 0 85 ++ indexSeq 85 2 := by
    rw [← indexSeq_append 0 85 2] at h
    exact h
  have hcancel := List.append_cancel_left hsplit
  exact (by decide : 0 ≠ 85) (indexSeq_start_inj (by decide : 0 < 2) hcancel)

/-- fork.py:1118. The 87-chain is queues plus 439 Gwei, not the
85-chain's queues plus 427 Gwei. -/
theorem eightyseven_chain_credits_are_not_eightyfive :
    439 * GWEI_TO_WEI ≠ 427 * GWEI_TO_WEI := by
  simp [GWEI_TO_WEI]

/-- fork.py:1118. The 87-chain is not the 85-chain credit
(`items b1 + 420e9`). -/
theorem eightyseven_chain_credits_are_not_thirty_five_pairs :
    432 * GWEI_TO_WEI ≠ 420 * GWEI_TO_WEI := by
  simp [GWEI_TO_WEI]



/-- Capella:452/480. The thirty-eighth payload's first sweep is stamped at
`start+87`, not restarted at 0. -/
theorem thirty_eighth_payload_sweep_index_is_not_restart :
    (thirtyEighthPayloadContinueSweep 0).index ≠
      (thirtyEighthPayloadContinueSweepRestart 0).index :=
  thirtyEighthPayloadContinueSweep_ne_restart 0

/-- Mutant: stamp from Gloas:2016 visits=2. -/
theorem thirty_eighth_payload_sweep_index_is_not_visits :
    (thirtyEighthPayloadContinueSweep 0).index ≠
      (thirtyEighthPayloadContinueSweepFromVisits 0).index :=
  thirtyEighthPayloadContinueSweep_ne_visits 0

/-- Mutant: stamp from the 85-chain cursor. -/
theorem thirty_eighth_payload_sweep_index_is_not_eightyfive :
    (thirtyEighthPayloadContinueSweep 0).index ≠
      (thirtyEighthPayloadContinueSweepFromEightyFive 0).index :=
  thirtyEighthPayloadContinueSweep_ne_eightyfive 0

/-- Mutant: freeze after the thirty-seventh payload's second continued sweep. -/
theorem thirty_eighth_payload_sweep_index_is_not_eightysix :
    (thirtyEighthPayloadContinueSweep 0).index ≠
      (thirtyEighthPayloadContinueSweepFromEightySix 0).index :=
  thirtyEighthPayloadContinueSweep_ne_eightysix 0

/-- This payload's sweep cursor is `FLAG`, not the first payload `1|FLAG`. -/
theorem thirty_eighth_payload_sweep_validator_is_not_first_payload :
    (thirtyEighthPayloadContinueSweep 0).validatorIndex ≠
      (firstPayloadSkipTakeBreakCapWithdrawal 0).validatorIndex :=
  thirtyEighthPayloadContinueSweep_ne_first_payload_validator 0

/-- Mutant: reuse the thirty-seventh payload's second-sweep `1|FLAG`. -/
theorem thirty_eighth_payload_sweep_validator_is_not_thirty_seventh_second :
    (thirtyEighthPayloadContinueSweep 0).validatorIndex ≠
      (thirtySeventhPayloadContinueSweepSecond 0).validatorIndex :=
  thirtyEighthPayloadContinueSweep_ne_thirty_seventh_second_validator 0

...
Single bytes of the pinned builder_exits runtime are changed and the *same*
`PDrain1.drainFacts` the parent is registered against is re-evaluated. It
must come out `false`.

Two drain-only bytes are cut. Neither is on a path P-SUBMIT-1 or
P-CONTROL-1 observe:
...
* **the drained record size** — the `68` at offset 450, the `PUSH1 RECORD_SIZE`
...
theorem pinned_drain_bytes :
    ...
    ∧ exitRuntime.get! exitRecordSizeIdx = 0x44 := by
  native_decide
...
theorem mutants_differ_in_one_byte :
    ...
    ∧ recSizeMutatedExit.get! exitRecordSizeIdx = 0x40 := by
  native_decide

/--
**Kill-line.** Each mutant refutes `drainFacts`, the exact conjunction
`pdrain1_bytecode_parent` is registered against. The deposit runtime is
left pinned in each case, so the failure is attributable to the cut exit
bytes.
-/
theorem mutant_refutes_parent :
    drainFacts depositRuntime capMutatedExit = false
    ∧ drainFacts depositRuntime recSizeMutatedExit = false := by
  native_decide
...
theorem pinned_satisfies_what_mutants_break :
    ...
    ∧ exitUnderCapFifoFact recSizeMutatedExit = false := by
  native_decide

/--
**The parent is not a sibling.** Both drain mutants leave
...
theorem drain_mutants_leave_siblings_intact :
    ...
    ∧ Eip8282.Audit.Guarantees.PControl1.controlFacts depositRuntime recSizeMutatedExit = true := by
  native_decide

...
/-! Installed code and existing accounts survive every completed pinned-runtime
message call. Storage may change. No queue, fee, funding, or resource adequacy
premise is used; rollback outcomes retain their literal wrapper semantics. -/
namespace Eip8282.Audit.Integrator.RuntimeCodePreservation
...
def Preserved (before after : AccountMap .EVM) (a : AccountAddress) : Prop :=
  ∀ old, before.get? a = some old →
    ∃ current, after.get? a = some current ∧ current.code = old.code

private theorem refl (w : AccountMap .EVM) (a : AccountAddress) : Preserved w w a :=
  fun old h => ⟨old,h,rfl⟩

private theorem trans {u v w : AccountMap .EVM} {a : AccountAddress}
    (h : Preserved u v a) (g : Preserved v w a) : Preserved u w a := by
  intro old ho
  obtain ⟨mid,hm,hcode⟩ := h old ho
  obtain ⟨post,hp,hcode'⟩ := g mid hm
  exact ⟨post,hp,hcode'.trans hcode⟩
...
private theorem sstore (st : EvmYul.State .EVM) (key value : UInt256) (a : AccountAddress) :
  ...
  by_cases he : st.executionEnv.codeOwner = a
  · intro old ho
    have hown : st.accountMap.get? st.executionEnv.codeOwner = some old := by simpa only [he] using ho
    rw [SystemSpec.accountMap_sstore hown]
    refine ⟨old.updateStorage key value, ?_, by unfold Account.updateStorage; split <;> rfl⟩
    rw [he]
  ...
  · cases ha : st.accountMap.get? st.executionEnv.codeOwner with
    ...
    | some acc =>
        ...
        intro old ho
        refine ⟨old, ?_, rfl⟩
        rw [show (st.accountMap.insert st.executionEnv.codeOwner (acc.updateStorage key value)).get? a = st.accountMap.get? a from by
          exact (Std.TreeMap.getElem?_insert (t := st.accountMap) (k := st.executionEnv.codeOwner) (a := a) (v := acc.updateStorage key value)).trans (by simp [he])]
        exact ho
...
/-- Every opcode actually admitted by the runtime image preserves code, even
SSTORE at the very account whose installed code is being preserved. -/
...
private theorem execution_preserved (c : MessageCall.Context) {kind : Eip8282.Audit.Model.Kind}
    ...
    Preserved c.entryWorld world c.target := by
  ...
  | ok result =>
    ...
    | success post o =>
      ...
      obtain ⟨hw,ho⟩ := hx
      have hp := x_preserved hat he c.target
      have hm := congrArg (fun t : Created × World × UInt256 × Substate => t.2.1) hw
      dsimp only at hm
      rw [hm] at hp
      exact hp

theorem theta_existing_code (c : MessageCall.Context) {kind : Eip8282.Audit.Model.Kind}
    {old : Account .EVM} {created : Created} {world : World} {gas : UInt256}
    {substate : Substate} {success : Bool} {out : ByteArray}
    (hcode : c.code = runtimeCode kind) (hold : c.world.get? c.target = some old)
    (hr : c.result = .ok (created,world,gas,substate,success,out)) :
    ∃ current, world.get? c.target = some current ∧ current.code = old.code := by
  ...
  | ok result =>
    ...
    | success state data =>
      ...
      have hp := execution_preserved c hcode he
      obtain ⟨entry,hen,henc,_⟩ := TransferFrame.entry_existing_account c hold
      obtain ⟨current,hcur,hcode'⟩ := hp entry hen
      simp only [MessageCall.Context.settle, he] at hr
      ...
      split
      · exact ⟨old,hold,rfl⟩
      · exact ⟨current,hcur,hcode'.trans henc⟩

#print axioms raw_preserved
#print axioms accepted_preserved
#print axioms x_preserved
#print axioms theta_existing_code
end Eip8282.Audit.Integrator.RuntimeCodePreservation

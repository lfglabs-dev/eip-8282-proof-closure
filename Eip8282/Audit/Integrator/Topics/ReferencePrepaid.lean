...
namespace Eip8282.Audit.Integrator.ReferencePrepaidFailure
...
theorem settled {deposit exit : TransactionAppendBudget.Receipt} {Hash LoadError : Type} [DecidableEq Hash]
    ...
    (actual : ReferenceCheckedAccountEvaluator.eval accountsParent destinations parent ByteArray.empty fuel
      (entry (call kind transaction) emptyHash accountsParent (prepaid emptyHash accountsParent before transaction).2 codeParent shouldTransfer).2.accounts
      (initial (CallBridge.codeCall (call kind transaction) (code kind transaction) 0) (entry (call kind transaction) emptyHash accountsParent (prepaid emptyHash accountsParent before transaction).2 codeParent shouldTransfer).2.storage)
    ...
    let live := {(entry (call kind transaction) emptyHash accountsParent (prepaid emptyHash accountsParent before transaction).2 codeParent shouldTransfer).2 with accounts := finalAccounts, storage := view.storage}
    ...
    ∃ receipt,
      ReferenceCheckedFrameOutcome.settle (prepaid emptyHash accountsParent before transaction).2.storage [] warm (.failed fault view partialWarm partialMeter output) = .returned receipt ∧
  ...
  refine ⟨ready.1,?_⟩
  exact ReferenceCheckpointFailure.settled kind transaction history admission recipient emptyHash accountsParent
    (prepaid emptyHash accountsParent before transaction).2 codeParent shouldTransfer context ready.2.1 ready.2.2.1 actual ready.2.2.2 warmRelated grant calldata

#print axioms settled
end Eip8282.Audit.Integrator.ReferencePrepaidFailure

end

section

/-! ## ReferencePrepaidGuarantees -/

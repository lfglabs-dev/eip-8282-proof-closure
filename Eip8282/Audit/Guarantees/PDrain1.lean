...
theorem pdrain1_kill_line_opcodes :
    opcodeAt Fifo.exitClamp 18 =
      ...
      Fifo.recordSize .exit = 68 :=
  ⟨Fifo.exit_clamp_opcode_PUSH1_cap, rfl, Fifo.exit_clamp_rel_cap,
    Fifo.deposit_clamp_opcode_PUSH1_cap, rfl, Fifo.deposit_clamp_rel_cap,
    Footprint.deposit_update_head_PUSH1_QUEUE_HEAD,
    Footprint.deposit_kill_line_immediate_pc,
    Footprint.exit_update_head_PUSH1_QUEUE_HEAD,
    Footprint.exit_kill_line_immediate_pc,
    by rfl, rfl, Fifo.recordSize_exit⟩

/-- D1: system `SSTORE` keys ⊆ `{SLOT_EXCESS, SLOT_COUNT, QUEUE_HEAD,
QUEUE_TAIL}`; `∀ n ≥ 4`, a system CFG store list leaves that slot equal
to the pre-image. Kill-line: `PUSH1 QUEUE_HEAD` at deposit 483 / exit 313. -/
theorem pdrain1_d1_footprint_forall :=
  And.intro Footprint.system_sstore_keys_subset
    Footprint.stale_slots_preserved

/-- D2: `n = min(tail-head, capOf)` wrap-free under WellFormed; oldest
window; full drain pointers `(0,0)`; partial `HEAD += n`, `TAIL`
unchanged; caps 64/16. Kill-line: PUSH1 cap at 304/244. Not a `Ξ`
reduction. -/
theorem pdrain1_d2_fifo_forall :=
  And.intro Fifo.fifo_system_spec
    Fifo.fifo_pointers_both_kinds

/-- D3: deposit amount BE storage → LE return `∀` drained index; exit
records are source‖pubkey with no recode; a user fee quote does not move
`QUEUE_HEAD` / `QUEUE_TAIL`. -/
theorem pdrain1_d3_encode_forall :=
  And.intro Encode.deposit_amount_be_to_le <|
  And.intro Encode.exit_record_encoding
    Encode.user_fee_does_not_move_pointers

/--
**P-DRAIN-1 parent.** CFG-level `∀` under `WellFormed` / `CallHyp`
(gas ≥ 30M, fuel ≥ 80000, system caller `isUser = false`) for SSTORE
footprint, FIFO count/pointers, and deposit BE→LE encode, plus the Wave-6
`pdrain1_bytecode_parent` traces. Not `unfold systemCall`. Not `Ξ ↔ Model`.
Does not claim `Ξ` computes FIFO for every excess.
...
deposit HEAD@483 (→9 and →196), and exit HEAD@313 mutants. The opcode
conjuncts name those mutated PCs on the CFG fragments.
-/
theorem pdrain1_forall_parent :=

...
## Intrinsic admission and calldata floor

The proposed Amsterdam pin decomposes the execution intrinsic base into 12000 plus recipient/value charges; its cold recipient charge is 3000. The pinned EVMYulLean intrinsic calculation starts at 21000. These formulas must not be identified. The archived complete sources are in `direct-reference-admission-sources-20260910.json`.

`ReferenceIntrinsicGap` checks a local unsigned empty-data sample with gas limit12000. The zero-recipient/zero-access source floor equals12000 and satisfies both floor comparisons; the actual pinned intrinsicGas equals21000. This refutes the **floor-gates → old intrinsic admission** implication. It does not show full source validity, a recoverable signature, funding, constructor reachability, or canonical inclusion; no source execution result is claimed.

Functional impact: translating admission by reusing the old intrinsic gate can exclude source-admitted shapes or assign different entry gas, particularly zero-value calls such as fee quotes. Actual source success/OOG must be proved with its own allocation rules. Economic impact: the calldata floor and split execution/state allocation change minimum charges and usable resources; old transaction debit/refund formulas cannot substitute for the source accounting. Security impact: this is an exposed semantic-transport gap, not an exploit demonstrated against either EIP-8282 bytecode. Existing direct bytecode theorems remain valid in their stated evaluator domain; applying them to canonical Amsterdam requires the remaining source adapter.

The replacement **size producer** uses the source floor directly, without altering the public guarantees: `((data_length*4)+access_tokens)*16+(12000+recipient_cost) ≤ 16777216` implies `data_length ≤ 261956` for all nonnegative costs. `ReferenceAdmissionHistory` feeds that derived size fit into the existing actual history and committed-effect consumers. It does not claim equality of the complete validators. Full source validation, execution grants, state reservoir/refunds and canonical transaction extraction remain open obligations.

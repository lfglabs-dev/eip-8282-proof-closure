...
### ENTRY-REACH: the paths from `c.entry`

`Eip8282.Audit.EntryReach` is the first slice of the closure work: it follows
every complete `Ξ` call into both pinned runtimes from the entry machine
`c.entry` to the halting instruction it reaches, one theorem per endpoint
(`EntryReach.Deposit`: inhibited, bad size, paid getter, getter, underpay, amount
floor, stake, accepted append, system drain; `EntryReach.Exit`: the same minus
the two deposit-only checks). The machinery is a symbolic execution of the pinned
bytes — `Eip8282.Audit.SymExec` validates a pure-opcode table against
`EvmYul.step` once (`pureStep_sound`) and proves one block-soundness theorem
(`xRuns_symBlock`); `EntryReach/Blocks.lean` is generated by
`scripts/gen_blocks.py` and consists of `rfl` block shapes over kernel-checked
decodes of the pinned images; `EntryReach/Steps.lean` handles the effectful
instructions (`JUMPI`, `SSTORE`, `MSTORE`, `MSTORE8`, `CALLDATACOPY`, `LOG0`,
`RETURN`, `REVERT`, `STOP`) against EVMYulLean's own `Z` and `step`. Nothing in it
runs `Ξ`, and nothing uses `native_decide`.

The bridge `EntryReach.xiHalts_of_ends` turns a completed path into the
...
(`callerWord`, `excessWord`, `cdsizeWord`, `valueWord`, the fee word the loop
computes), not yet the model's `userCall` arguments; the fee loop's termination
on 256-bit words (`FeeLoopEnds`) is a premise; so are gas, fuel and write
permission. Relating those words to `Model.step`, deriving the budgets from
`AdmissibleCall`, and then proving `ExitAgrees`/`PostStateAgrees` at each
...
The ENTRY-REACH layer, likewise without `Ξ` or FFI; its `#print axioms` lines
in `Eip8282.Audit.Trust` must also show nothing beyond the three foundational
axioms:

```bash
lake build Eip8282.Audit.EntryReach
```
...
- `Eip8282/Audit/` — abstract model, `Bytecode` pins, the `EvmRunner` Ξ driver,
  ...
  `A-ABSTRACT-TX` gap, the `SymExec` / `EntryReach` symbolic execution of the
  pinned runtimes from `c.entry` (ENTRY-REACH), trust report, facade
